...
use Mailster\Aws3\GuzzleHttp\Cookie\CookieJar;
 ...
 */
class Client implements \Mailster\Aws3\GuzzleHttp\ClientInterface
{
    ...
    public function __construct(array $config = [])
    {
        if (!isset($config['handler'])) {
            $config['handler'] = \Mailster\Aws3\GuzzleHttp\HandlerStack::create();
        } elseif (!is_callable($config['handler'])) {
            throw new \InvalidArgumentException('handler must be a callable');
        }
        // Convert the base_uri to a UriInterface
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \Mailster\Aws3\GuzzleHttp\Psr7\uri_for($config['base_uri']);
        }
    ...
    {
        $options[\Mailster\Aws3\GuzzleHttp\RequestOptions::SYNCHRONOUS] = true;
    ...
    public function requestAsync($method, $uri = '', array $options = [])
    {
        ...
        $request = new \Mailster\Aws3\GuzzleHttp\Psr7\Request($method, $uri, $headers, $body, $version);
    ...
    private function buildUri($uri, array $config)
    {
        ...
        $uri = \Mailster\Aws3\GuzzleHttp\Psr7\uri_for($uri === null ? '' : $uri);
        if (isset($config['base_uri'])) {
            $uri = \Mailster\Aws3\GuzzleHttp\Psr7\UriResolver::resolve(\Mailster\Aws3\GuzzleHttp\Psr7\uri_for($config['base_uri']), $uri);
        }
    ...
    private function configureDefaults(array $config)
    {
        ...
        if (!empty($config['cookies']) && $config['cookies'] === true) {
            $this->config['cookies'] = new \Mailster\Aws3\GuzzleHttp\Cookie\CookieJar();
        }
     ...
     */
    private function transfer(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request, array $options)
    {
        // save_to -> sink
        ...
        try {
            return \Mailster\Aws3\GuzzleHttp\Promise\promise_for($handler($request, $options));
        } catch (\Exception $e) {
     ...
     */
    private function applyOptions(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request, array &$options)
    {
        $modify = ['set_headers' => []];
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \Mailster\Aws3\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
            $options['_conditional']['Content-Type'] = 'application/x-www-form-urlencoded';
        }
        if (isset($options['multipart'])) {
            $options['body'] = new \Mailster\Aws3\GuzzleHttp\Psr7\MultipartStream($options['multipart']);
            unset($options['multipart']);
        }
        if (isset($options['json'])) {
            $options['body'] = \Mailster\Aws3\GuzzleHttp\json_encode($options['json']);
            unset($options['json']);
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Mailster\Aws3\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \Mailster\Aws3\GuzzleHttp\Psr7\_caseless_remove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \Mailster\Aws3\GuzzleHttp\Psr7\stream_for($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \Mailster\Aws3\GuzzleHttp\Psr7\_caseless_remove(['Authorization'], $modify['set_headers']);
        ...
        $request = \Mailster\Aws3\GuzzleHttp\Psr7\modify_request($request, $modify);
