 ...
 */
class CommandPool implements \Mailster\Aws3\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\Mailster\Aws3\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $this->each = new \Mailster\Aws3\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\Mailster\Aws3\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
