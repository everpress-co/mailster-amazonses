...
namespace Mailster\Aws3\Aws\Endpoint;

use Mailster\Aws3\Aws\Exception\UnresolvedEndpointException;
/**
 * Endpoint providers.
 *
...
class EndpointProvider
{
    ...
    public static function resolve(callable $provider, array $args = [])
    {
        ...
        throw new \Mailster\Aws3\Aws\Exception\UnresolvedEndpointException('Unable to resolve an endpoint using the provider arguments: ' . json_encode($args) . '. Note: you can provide an "endpoint" ' . 'option to a client constructor to bypass invoking an endpoint ' . 'provider.');
    }
    /**
    ...
    {
        return \Mailster\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider();
    }
    /**
    ...
    {
        return new \Mailster\Aws3\Aws\Endpoint\PatternEndpointProvider($patterns);
    }
}
