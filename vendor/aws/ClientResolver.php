...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        if ($value) {
            $decider = \Mailster\Aws3\Aws\RetryMiddleware::createDefaultDecider($value);
            $list->appendSign(\Mailster\Aws3\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            $args['credentials'] = \Mailster\Aws3\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \Mailster\Aws3\Aws\Credentials\CredentialProvider::fromCredentials(new \Mailster\Aws3\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === false) {
            $args['credentials'] = \Mailster\Aws3\Aws\Credentials\CredentialProvider::fromCredentials(new \Mailster\Aws3\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof CacheInterface) {
            $args['credentials'] = \Mailster\Aws3\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . 'Mailster\\Aws3\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \Mailster\Aws3\Aws\Api\Service(\Mailster\Aws3\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \Mailster\Aws3\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \Mailster\Aws3\Aws\Api\Service::createErrorParser($api->getProtocol());
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            $result = \Mailster\Aws3\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme']]);
    ...
    public static function _apply_serializer($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        $list->prependBuild(\Mailster\Aws3\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        if ($value !== false) {
            $list->interpose(new \Mailster\Aws3\Aws\TraceMiddleware($value === true ? [] : $value));
        }
    }
    public static function _apply_stats($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\Mailster\Aws3\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \Mailster\Aws3\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        if ($value === false) {
            return;
        }
        $validator = $value === true ? new \Mailster\Aws3\Aws\Api\Validator() : new \Mailster\Aws3\Aws\Api\Validator($value);
        $list->appendValidate(\Mailster\Aws3\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \Mailster\Aws3\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        $args['handler'] = new \Mailster\Aws3\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        if (!is_array($value)) {
            $value = [$value];
        }
        ...
        array_unshift($value, 'aws-sdk-php/' . \Mailster\Aws3\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\Mailster\Aws3\Aws\CommandInterface $command, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', implode(' ', array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \Mailster\Aws3\Aws\HandlerList $list)
    {
        $enabled = false;
        $generator = null;
        if (is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\Mailster\Aws3\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        return \Mailster\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \Mailster\Aws3\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \Mailster\Aws3\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : call_user_func(\Mailster\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \Mailster\Aws3\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
