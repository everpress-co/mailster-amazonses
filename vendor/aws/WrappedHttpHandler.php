...
class WrappedHttpHandler
{
     ...
     */
    public function __invoke(\Mailster\Aws3\Aws\CommandInterface $command, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $fn = $this->httpHandler;
        ...
        return \Mailster\Aws3\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\Mailster\Aws3\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \Mailster\Aws3\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\Mailster\Aws3\Aws\CommandInterface $command, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request, \Mailster\Aws3\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \Mailster\Aws3\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request, \Mailster\Aws3\Aws\CommandInterface $command, array $stats)
    {
