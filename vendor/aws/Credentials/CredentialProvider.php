...
use Aws;
use Mailster\Aws3\Aws\CacheInterface;
...
class CredentialProvider
{
     ...
     * @return callable
     */
    public static function fromCredentials(\Mailster\Aws3\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \Mailster\Aws3\GuzzleHttp\Promise\promise_for($creds);
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\Mailster\Aws3\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
     ...
     * @return callable
     */
    public static function cache(callable $provider, \Mailster\Aws3\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof CredentialsInterface && !$found->isExpired()) {
                return \Mailster\Aws3\GuzzleHttp\Promise\promise_for($found);
            }
            return $provider()->then(function (\Mailster\Aws3\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \Mailster\Aws3\GuzzleHttp\Promise\promise_for(new \Mailster\Aws3\Aws\Credentials\Credentials($key, $secret, getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \Mailster\Aws3\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    {
        return new \Mailster\Aws3\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \Mailster\Aws3\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \Mailster\Aws3\GuzzleHttp\Promise\promise_for(new \Mailster\Aws3\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    {
        if (!empty(getenv(\Mailster\Aws3\Aws\Credentials\EcsCredentialProvider::ENV_URI))) {
    ...
    {
        return new \Mailster\Aws3\GuzzleHttp\Promise\RejectedPromise(new \Mailster\Aws3\Aws\Exception\CredentialsException($msg));
    }
}
