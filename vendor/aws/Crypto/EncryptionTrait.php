...
use Mailster\Aws3\GuzzleHttp\Psr7\AppendStream;
...
trait EncryptionTrait
{
     ...
     * @internal
     */
    protected function encrypt(\Mailster\Aws3\GuzzleHttp\Psr7\Stream $plaintext, array $cipherOptions, \Mailster\Aws3\Aws\Crypto\MaterialsProvider $provider, \Mailster\Aws3\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\Mailster\Aws3\Aws\Crypto\MaterialsProvider::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $provider->encryptCek($cek, $materialsDescription);
        unset($cek);
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::IV_HEADER] = base64_encode($cipherOptions['Iv']);
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::KEY_WRAP_ALGORITHM_HEADER] = $provider->getWrapAlgorithmName();
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER] = $aesName;
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_LENGTH_HEADER] = strlen($plaintext);
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_MD5_HEADER] = base64_encode(md5($plaintext));
        $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER] = json_encode($materialsDescription);
        if (!empty($cipherOptions['Tag'])) {
            $envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] = strlen($cipherOptions['Tag']) * 8;
        }
     ...
     * @internal
     */
    protected function getEncryptingStream(\Mailster\Aws3\GuzzleHttp\Psr7\Stream $plaintext, $cek, &$cipherOptions)
    {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['TagLength'] = 16;
                $cipherTextStream = new \Mailster\Aws3\Aws\Crypto\AesGcmEncryptingStream($plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'], $cipherOptions['KeySize']);
                $appendStream = new \Mailster\Aws3\GuzzleHttp\Psr7\AppendStream([$cipherTextStream->createStream()]);
                $cipherOptions['Tag'] = $cipherTextStream->getTag();
                $appendStream->addStream(\Mailster\Aws3\GuzzleHttp\Psr7\stream_for($cipherOptions['Tag']));
            ...
            default:
                ...
                $cipherTextStream = new \Mailster\Aws3\Aws\Crypto\AesEncryptingStream($plaintext, $cek, $cipherMethod);
