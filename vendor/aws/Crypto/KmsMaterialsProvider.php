...
use Mailster\Aws3\Aws\Kms\KmsClient;
/**
 * Uses KMS to supply materials for encrypting and decrypting data.
 */
class KmsMaterialsProvider extends \Mailster\Aws3\Aws\Crypto\MaterialsProvider
{
    private $kmsClient;
    private $kmsKeyId;
    /**
     ...
     *                         and decrypting keys.
     */
    public function __construct(\Mailster\Aws3\Aws\Kms\KmsClient $kmsClient, $kmsKeyId = null)
    {
        $this->kmsClient = $kmsClient;
        $this->kmsKeyId = $kmsKeyId;
    }
    public function fromDecryptionEnvelope(\Mailster\Aws3\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect kms_cmk_id from an' . ' empty materials description.');
        }
        $materialsDescription = json_decode($envelope[\Mailster\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true);
        ...
        return new \Mailster\Aws3\Aws\Crypto\KmsMaterialsProvider($this->kmsClient, $materialsDescription['kms_cmk_id']);
    }
    /**
