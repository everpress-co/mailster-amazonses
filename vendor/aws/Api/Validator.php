...
class Validator
{
     ...
     */
    public function validate($name, \Mailster\Aws3\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\Mailster\Aws3\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\Mailster\Aws3\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\Mailster\Aws3\Aws\Api\ListShape $shape, $value)
    {
        if (!is_array($value)) {
            $this->addError('must be an array. Found ' . \Mailster\Aws3\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\Mailster\Aws3\Aws\Api\MapShape $shape, $value)
    {
        if (!$this->checkAssociativeArray($value)) {
            return;
        }
    ...
    private function check_blob(\Mailster\Aws3\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => true, 'integer' => true, 'double' => true, 'resource' => true];
        $type = gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'Mailster\\Aws3\\GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \Mailster\Aws3\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\Mailster\Aws3\Aws\Api\Shape $shape, $value)
    {
        if (!is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \Mailster\Aws3\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\Mailster\Aws3\Aws\Api\Shape $shape, $value)
    {
        if (!is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \Mailster\Aws3\Aws\describe_type($value));
        }
    }
    private function check_string(\Mailster\Aws3\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \Mailster\Aws3\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \Mailster\Aws3\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\Mailster\Aws3\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        if (!is_array($value) || isset($value[0])) {
            $this->addError('must be an associative array. Found ' . \Mailster\Aws3\Aws\describe_type($value));
            return false;
        }
        return true;
    }
