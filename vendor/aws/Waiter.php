 ...
 */
class Waiter implements \Mailster\Aws3\GuzzleHttp\Promise\PromisorInterface
{
     ...
     */
    public function __construct(\Mailster\Aws3\Aws\AwsClientInterface $client, $name, array $args = [], array $config = [])
    {
        $this->client = $client;
        $this->name = $name;
        $this->args = $args;
        ...
        return \Mailster\Aws3\GuzzleHttp\Promise\coroutine(function () {
            ...
            for ($state = 'retry', $attempt = 1; $state === 'retry'; $attempt++) {
                ...
                if ($state === 'success') {
                    ...
                    (yield new \Mailster\Aws3\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException($msg)));
                } elseif ($state === 'retry' && $attempt >= $this->config['maxAttempts']) {
                    $state = 'failed';
                    (yield new \Mailster\Aws3\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException("The {$this->name} waiter failed after attempt #{$attempt}.")));
                }
            }
        });
    }
