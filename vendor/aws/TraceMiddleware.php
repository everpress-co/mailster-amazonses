...
class TraceMiddleware
{
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\Mailster\Aws3\Aws\CommandInterface $command, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\Mailster\Aws3\Aws\CommandInterface $cmd)
    {
        return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $cmd->toArray()];
    }
    private function requestArray(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\Mailster\Aws3\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    private function streamStr(\Mailster\Aws3\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\Mailster\Aws3\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\Mailster\Aws3\Aws\CommandInterface $command)
    {
