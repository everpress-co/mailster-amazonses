...
use Mailster\Aws3\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \Mailster\Aws3\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
    ...
    public function presign(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request, \Mailster\Aws3\Aws\Credentials\CredentialsInterface $credentials, $expires, array $options = [])
    {
     ...
     */
    public static function convertPostToGet(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\Mailster\Aws3\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \Mailster\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \Mailster\Aws3\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \Mailster\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    protected function getPresignedPayload(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request, \Mailster\Aws3\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \Mailster\Aws3\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\Mailster\Aws3\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \Mailster\Aws3\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
