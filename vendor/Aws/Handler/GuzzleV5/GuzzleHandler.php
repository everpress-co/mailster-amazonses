...
use Mailster\Aws3\GuzzleHttp\Event\EndEvent;
use Mailster\Aws3\GuzzleHttp\Exception\ConnectException;
use Mailster\Aws3\GuzzleHttp\Exception\RequestException;
use Mailster\Aws3\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use Mailster\Aws3\GuzzleHttp\Promise;
use Mailster\Aws3\GuzzleHttp\Psr7\Response as Psr7Response;
use Mailster\Aws3\GuzzleHttp\Stream\Stream;
use Mailster\Aws3\Psr\Http\Message\RequestInterface as Psr7Request;
...
class GuzzleHandler
{
     ...
     */
    public function __construct(\Mailster\Aws3\GuzzleHttp\ClientInterface $client = null)
    {
        $this->client = $client ?: new \Mailster\Aws3\GuzzleHttp\Client();
    }
    /**
    ...
    public function __invoke(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \Mailster\Aws3\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\Mailster\Aws3\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof Psr7StreamInterface) {
                        $exception->getResponse()->setBody(\Mailster\Aws3\GuzzleHttp\Stream\Stream::factory(file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \Mailster\Aws3\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\Mailster\Aws3\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof Psr7StreamInterface ? new \Mailster\Aws3\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\Mailster\Aws3\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \Mailster\Aws3\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \Mailster\Aws3\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\Mailster\Aws3\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \Mailster\Aws3\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \Mailster\Aws3\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
