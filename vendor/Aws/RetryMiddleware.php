...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3)
    {
        ...
        return function ($retries, \Mailster\Aws3\Aws\CommandInterface $command, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request, \Mailster\Aws3\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors) {
     ...
     */
    public function __invoke(\Mailster\Aws3\Aws\CommandInterface $command, \Mailster\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
        $retries = 0;
        $requestStats = [];
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$g) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($retries, $command, $request, null, $value)) {
                    return \Mailster\Aws3\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
