...
use Mailster\Aws3\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use Mailster\Aws3\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use Mailster\Aws3\Aws\ClientSideMonitoring\Configuration;
...
use Mailster\Aws3\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use Mailster\Aws3\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use Mailster\Aws3\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use Mailster\Aws3\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use Mailster\Aws3\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use Mailster\Aws3\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use Mailster\Aws3\Aws\EndpointDiscovery\ConfigurationInterface;
use Mailster\Aws3\Aws\EndpointDiscovery\ConfigurationProvider;
use Mailster\Aws3\Aws\EndpointV2\EndpointDefinitionProvider;
use Mailster\Aws3\Aws\Exception\InvalidRegionException;
use Mailster\Aws3\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use Mailster\Aws3\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use Mailster\Aws3\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use Mailster\Aws3\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use Mailster\Aws3\Aws\Signature\SignatureProvider;
use Mailster\Aws3\Aws\Endpoint\EndpointProvider;
use Mailster\Aws3\Aws\Token\Token;
use Mailster\Aws3\Aws\Token\TokenInterface;
use Mailster\Aws3\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            ...
            throw new IAE('Credentials must be an instance of ' . 'Mailster\\Aws3\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    ...
    public static function _apply_token($value, array &$args)
    {
        ...
        if ($value instanceof Token) {
            ...
            throw new IAE('Token must be an instance of ' . 'Mailster\\Aws3\\Aws\\Token\\TokenInterface, an associative ' . 'array that contains "token" and an optional "expires" ' . 'key-value pairs, a token provider function, or false.');
        }
    }
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \Mailster\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \Mailster\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \Mailster\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \Mailster\Aws3\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \Mailster\Aws3\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    public static function _apply_user_agent($inputUserAgent, array &$args, HandlerList $list)
    {
        ...
        if (\defined('Mailster\\Aws3\\HHVM_VERSION')) {
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \Mailster\Aws3\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
...
EOT;
    ...
    private static function isValidService($service)
    {
        ...
        $services = \Mailster\Aws3\Aws\manifest();
