...
namespace Mailster\Aws3\Aws\Multipart;

use Mailster\Aws3\Aws\AwsClientInterface as Client;
use Mailster\Aws3\GuzzleHttp\Psr7;
use InvalidArgumentException as IAE;
use Mailster\Aws3\Psr\Http\Message\StreamInterface as Stream;
abstract class AbstractUploader extends \Mailster\Aws3\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\Mailster\Aws3\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\Mailster\Aws3\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \Mailster\Aws3\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
    ...
    private function determineSource($source)
    {
        ...
        if (is_string($source)) {
            $source = \Mailster\Aws3\GuzzleHttp\Psr7\try_fopen($source, 'r');
        }
        // Create a source stream.
        $stream = \Mailster\Aws3\GuzzleHttp\Psr7\stream_for($source);
