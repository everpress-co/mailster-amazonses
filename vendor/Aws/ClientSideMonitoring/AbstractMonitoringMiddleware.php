...
abstract class AbstractMonitoringMiddleware implements MonitoringMiddlewareInterface
{
    ...
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) \floor(\microtime(\true) * 1000), 'UserAgent' => \substr($request->getHeaderLine('User-Agent') . ' ' . \Mailster\Aws3\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
