...
class EcsCredentialProvider
{
    ...
    {
        $client = $this->client;
        $request = new \Mailster\Aws3\GuzzleHttp\Psr7\Request('GET', self::getEcsUri());
        return $client($request, ['timeout' => $this->timeout])->then(function (\Mailster\Aws3\Psr\Http\Message\ResponseInterface $response) {
            $result = $this->decodeResult((string) $response->getBody());
            return new \Mailster\Aws3\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], strtotime($result['Expiration']));
        })->otherwise(function ($reason) {
            $reason = is_array($reason) ? $reason['exception'] : $reason;
            $msg = $reason->getMessage();
            throw new \Mailster\Aws3\Aws\Exception\CredentialsException("Error retrieving credential from ECS ({$msg})");
        });
    }
    /**
    ...
    private function decodeResult($response)
    {
        ...
        if (!isset($result['AccessKeyId'])) {
            throw new \Mailster\Aws3\Aws\Exception\CredentialsException('Unexpected ECS credential value');
        }
        return $result;
    }
}
