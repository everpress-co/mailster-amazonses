...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\Mailster\Aws3\Psr\Http\Message\RequestInterface $request, \Mailster\Aws3\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \Mailster\Aws3\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \Mailster\Aws3\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
    ...
    private function headers(\Mailster\Aws3\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
