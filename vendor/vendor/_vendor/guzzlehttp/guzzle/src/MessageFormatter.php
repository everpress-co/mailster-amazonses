...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\Mailster\Psr\Http\Message\RequestInterface $request, \Mailster\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \Mailster\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \Mailster\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
     ...
     * @return string
     */
    private function headers(\Mailster\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
