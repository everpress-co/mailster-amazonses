...
namespace Mailster\Aws\DynamoDb;

use Mailster\Aws\Api\Parser\Crc32ValidatingParser;
 ...
 */
class DynamoDbClient extends \Mailster\Aws\AwsClient
{
    ...
    {
        $handler = \Mailster\Aws\DynamoDb\SessionHandler::fromClient($this, $config);
        $handler->register();
        return $handler;
    }
    /** @internal */
    public static function _applyRetryConfig($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \Mailster\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $list->appendSign(\Mailster\Aws\Middleware::retry(\Mailster\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1, ['error_codes' => ['TransactionInProgressException']]), function ($retries) {
                    return $retries ? \Mailster\Aws\RetryMiddleware::exponentialDelay($retries) / 2 : 0;
                }, isset($args['stats']['retries']) ? (bool) $args['stats']['retries'] : \false), 'retry');
            } else {
                $list->appendSign(\Mailster\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'transient_error_codes' => ['TransactionInProgressException']]), 'retry');
            }
        }
    }
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        ...
        $args['parser'] = new \Mailster\Aws\Api\Parser\Crc32ValidatingParser($args['parser']);
    }
}
