...
class WriteRequestBatch
{
    ...
    public function flush($untilEmpty = \true)
    {
        ...
        while ($this->queue && $keepFlushing) {
            ...
            $pool = new \Mailster\Aws\CommandPool($this->client, $commands, ['before' => $this->config['before'], 'concurrency' => $this->config['pool_size'], 'fulfilled' => function (\Mailster\Aws\ResultInterface $result) {
                // Re-queue any unprocessed items
                if ($result->hasKey('UnprocessedItems')) {
                    $this->retryUnprocessed($result['UnprocessedItems']);
                }
            }, 'rejected' => function ($reason) {
                if ($reason instanceof \Mailster\Aws\Exception\AwsException) {
