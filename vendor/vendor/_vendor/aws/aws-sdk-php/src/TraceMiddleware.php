...
class TraceMiddleware
{
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\Mailster\Aws\CommandInterface $cmd)
    {
        return ['instance' => \spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $cmd->toArray()];
    }
    private function requestArray(\Mailster\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\Mailster\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    {
        return $value instanceof \Mailster\Aws\ResultInterface ? ['instance' => \spl_object_hash($value), 'data' => $value->toArray()] : $value;
    }
    ...
    private function streamStr(\Mailster\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\Mailster\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = \fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\Mailster\Aws\CommandInterface $command)
    {
