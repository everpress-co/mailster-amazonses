...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \Mailster\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return new \Mailster\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \Mailster\GuzzleHttp\default_user_agent();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return \Mailster\GuzzleHttp\Client::getDefaultUserAgent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
...
function guzzle_major_version()
{
    ...
    if (\defined('\\GuzzleHttp\\ClientInterface::VERSION')) {
        $version = (string) \Mailster\GuzzleHttp\ClientInterface::VERSION;
        if ($version[0] === '6') {
            return $cache = 6;
        }
        if ($version[0] === '5') {
    ...
    } elseif (\method_exists(\Mailster\GuzzleHttp\Client::class, 'sendRequest')) {
        return $cache = 7;
    }
 ...
 * @throws \RuntimeException
 */
function serialize(\Mailster\Aws\CommandInterface $command)
{
    $request = null;
    $handlerList = $command->getHandlerList();
    // Return a mock result.
    $handlerList->setHandler(function (\Mailster\Aws\CommandInterface $_, \Mailster\Psr\Http\Message\RequestInterface $r) use(&$request) {
        $request = $r;
        return new \Mailster\GuzzleHttp\Promise\FulfilledPromise(new \Mailster\Aws\Result([]));
    });
    \call_user_func($handlerList->resolve(), $command)->wait();
    if (!$request instanceof \Mailster\Psr\Http\Message\RequestInterface) {
