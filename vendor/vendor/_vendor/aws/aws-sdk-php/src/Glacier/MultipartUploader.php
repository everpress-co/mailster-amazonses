...
namespace Mailster\Aws\Glacier;

use Mailster\Aws\CommandInterface;
use Mailster\Aws\HashingStream;
use Mailster\Aws\Multipart\AbstractUploader;
use Mailster\Aws\Multipart\UploadState;
use Mailster\Aws\PhpHash;
use Mailster\Aws\ResultInterface;
use Mailster\GuzzleHttp\Psr7;
use Mailster\Psr\Http\Message\StreamInterface as Stream;
/**
 * Encapsulates the execution of a multipart upload to Glacier.
 */
class MultipartUploader extends \Mailster\Aws\Multipart\AbstractUploader
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\Mailster\Aws\Glacier\GlacierClient $client, $vaultName, $uploadId, $accountId = '-')
    {
        $state = new \Mailster\Aws\Multipart\UploadState(['accountId' => $accountId, 'vaultName' => $vaultName, 'uploadId' => $uploadId]);
        ...
        $state->setStatus(\Mailster\Aws\Multipart\UploadState::INITIATED);
        return $state;
    }
    /**
     ...
     */
    public function __construct(\Mailster\Aws\Glacier\GlacierClient $client, $source, array $config = [])
    {
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = \Mailster\GuzzleHttp\Psr7\try_fopen($this->source->getMetadata('uri'), 'r');
            $body = $this->limitPartStream(\Mailster\GuzzleHttp\Psr7\stream_for($body));
            ...
            $body = \Mailster\GuzzleHttp\Psr7\stream_for();
            \Mailster\GuzzleHttp\Psr7\copy_to_stream($source, $body);
        }
    ...
    protected function handleResult(\Mailster\Aws\CommandInterface $command, \Mailster\Aws\ResultInterface $result)
    {
    ...
    {
        $treeHash = new \Mailster\Aws\Glacier\TreeHash();
        $archiveSize = 0;
     ...
     * @return Stream
     */
    private function decorateWithHashes(\Mailster\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Make sure that a tree hash is calculated.
        $stream = new \Mailster\Aws\HashingStream($stream, new \Mailster\Aws\Glacier\TreeHash(), function ($result) use(&$data) {
            $data['checksum'] = \bin2hex($result);
        });
        // Make sure that a linear SHA256 hash is calculated.
        $stream = new \Mailster\Aws\HashingStream($stream, new \Mailster\Aws\PhpHash('sha256'), function ($result) use(&$data) {
