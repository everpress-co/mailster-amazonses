 ...
 */
class GlacierClient extends \Mailster\Aws\AwsClient
{
    public function __construct(array $args)
    {
        ...
        $stack->appendBuild(\Mailster\Aws\Middleware::contentType(['UploadArchive', 'UploadPart']), 'glacier.content_type');
        $stack->appendInit(\Mailster\Aws\Middleware::sourceFile($this->getApi(), 'body', 'sourceFile'), 'glacier.source_file');
    }
    /**
     * {@inheritdoc}
     *
        ...
        return function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                ...
                if (($name === 'UploadArchive' || $name === 'UploadMultipartPart') && (!$command['checksum'] || !$command['ContentSHA256'])) {
                    ...
                    if (!$command['checksum']) {
                        $body = new \Mailster\Aws\HashingStream($body, new \Mailster\Aws\Glacier\TreeHash(), function ($result) use(&$request) {
                    ...
                    if (!$command['ContentSHA256']) {
                        $body = new \Mailster\Aws\HashingStream($body, new \Mailster\Aws\PhpHash('sha256'), function ($result) use($command) {
