...
use Mailster\GuzzleHttp\Event\EndEvent;
use Mailster\GuzzleHttp\Exception\ConnectException;
use Mailster\GuzzleHttp\Exception\RequestException;
use Mailster\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use Mailster\GuzzleHttp\Promise;
use Mailster\GuzzleHttp\Psr7\Response as Psr7Response;
use Mailster\GuzzleHttp\Stream\Stream;
use Mailster\Psr\Http\Message\RequestInterface as Psr7Request;
...
class GuzzleHandler
{
     ...
     */
    public function __construct(\Mailster\GuzzleHttp\ClientInterface $client = null)
    {
        $this->client = $client ?: new \Mailster\GuzzleHttp\Client();
    }
    /**
    ...
    public function __invoke(\Mailster\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \Mailster\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\Mailster\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof \Mailster\GuzzleHttp\Exception\RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof \Mailster\Psr\Http\Message\StreamInterface) {
                        $exception->getResponse()->setBody(\Mailster\GuzzleHttp\Stream\Stream::factory(\file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \Mailster\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\Mailster\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof \Mailster\Psr\Http\Message\StreamInterface ? new \Mailster\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (\is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\Mailster\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \Mailster\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \Mailster\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\Mailster\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \Mailster\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \Mailster\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
    private function prepareErrorData(\Exception $e)
    {
        $error = ['exception' => $e, 'connection_error' => \false, 'response' => null];
        if ($e instanceof \Mailster\GuzzleHttp\Exception\ConnectException) {
            $error['connection_error'] = \true;
        }
        if ($e instanceof \Mailster\GuzzleHttp\Exception\RequestException && $e->getResponse()) {
