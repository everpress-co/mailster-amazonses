...
namespace Mailster\Aws\EndpointDiscovery;
...
use Mailster\Aws\LruArrayCache;
...
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \Mailster\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            $config = \Mailster\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            ...
            if ($isRequired || $config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \Mailster\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \Mailster\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \Mailster\Aws\EndpointDiscovery\EndpointList([]);
                }
                ...
                $g = function ($value) use($cacheKey, $cmd, $identifiers, $isRequired, $originalUri, $request, &$endpoint, &$g) {
                    if ($value instanceof \Mailster\Aws\Exception\AwsException && ($value->getAwsErrorCode() == 'InvalidEndpointException' || $value->getStatusCode() == 421)) {
    ...
    private function discoverEndpoint($cacheKey, \Mailster\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \Mailster\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \Mailster\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\Mailster\Aws\Credentials\CredentialsInterface $creds, \Mailster\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\Mailster\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \Mailster\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\Mailster\Aws\Middleware::mapRequest(function (\Mailster\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \Mailster\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function handleInvalidEndpoint($cacheKey, $cmd, $identifiers, $isRequired, $originalUri, $request, $value, &$endpoint, &$g)
    {
        ...
        if ($endpointList instanceof \Mailster\Aws\EndpointDiscovery\EndpointList) {
    ...
    private function modifyRequest(\Mailster\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \Mailster\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\Mailster\Psr\Http\Message\UriInterface $uri, \Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
