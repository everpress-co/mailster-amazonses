 ...
 */
class CommandPool implements \Mailster\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\Mailster\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $mapFn = function ($commands) use($client, $before, $config) {
            foreach ($commands as $key => $command) {
                if (!$command instanceof \Mailster\Aws\CommandInterface) {
        ...
        $this->each = new \Mailster\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\Mailster\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
