...
use Mailster\Aws\Api\Validator;
use Mailster\Aws\Api\ApiProvider;
use Mailster\Aws\Api\Service;
use Mailster\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use Mailster\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use Mailster\Aws\ClientSideMonitoring\Configuration;
use Mailster\Aws\Credentials\Credentials;
use Mailster\Aws\Credentials\CredentialsInterface;
use Mailster\Aws\Endpoint\PartitionEndpointProvider;
use Mailster\Aws\EndpointDiscovery\ConfigurationInterface;
use Mailster\Aws\EndpointDiscovery\ConfigurationProvider;
use Mailster\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use Mailster\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use Mailster\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use Mailster\Aws\Signature\SignatureProvider;
use Mailster\Aws\Endpoint\EndpointProvider;
use Mailster\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \Mailster\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \Mailster\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \Mailster\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\Mailster\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\Mailster\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof \Mailster\Aws\Credentials\CredentialsInterface) {
            $args['credentials'] = \Mailster\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (\is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \Mailster\Aws\Credentials\CredentialProvider::fromCredentials(new \Mailster\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === \false) {
            $args['credentials'] = \Mailster\Aws\Credentials\CredentialProvider::fromCredentials(new \Mailster\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof \Mailster\Aws\CacheInterface) {
            $args['credentials'] = \Mailster\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
    ...
    {
        return \Mailster\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        if ($value === \false) {
            $value = new \Mailster\Aws\ClientSideMonitoring\Configuration(\false, \Mailster\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \Mailster\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \Mailster\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\Mailster\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\Mailster\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \Mailster\Aws\Api\Service(\Mailster\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \Mailster\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \Mailster\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            $result = \Mailster\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \Mailster\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_serializer($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        $list->prependBuild(\Mailster\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        if ($value !== \false) {
            $list->interpose(new \Mailster\Aws\TraceMiddleware($value === \true ? [] : $value));
        }
    }
    public static function _apply_stats($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\Mailster\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \Mailster\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        if ($value === \false) {
            return;
        }
        $validator = $value === \true ? new \Mailster\Aws\Api\Validator() : new \Mailster\Aws\Api\Validator($value);
        $list->appendValidate(\Mailster\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \Mailster\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        $args['handler'] = new \Mailster\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        if (!\is_array($value)) {
            $value = [$value];
        }
        ...
        \array_unshift($value, 'aws-sdk-php/' . \Mailster\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', \implode(' ', \array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        $enabled = \false;
        $generator = null;
        if (\is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\Mailster\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        $options = self::getEndpointProviderOptions($args);
        return \Mailster\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \Mailster\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \Mailster\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : \call_user_func(\Mailster\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \Mailster\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
