...
use Mailster\Aws\Retry\ConfigurationInterface;
use Mailster\Aws\Retry\QuotaManager;
use Mailster\Aws\Retry\RateLimiter;
use Mailster\Aws\Retry\RetryHelperTrait;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\Mailster\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (\extension_loaded('curl')) {
            $retryCurlErrors[\CURLE_RECV_ERROR] = \true;
        }
        return function ($attempts, \Mailster\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
            ...
            if ($isRetryable) {
                ...
                if ($attempts >= $maxAttempts) {
                    if (!empty($result) && $result instanceof \Mailster\Aws\Exception\AwsException) {
    ...
    public function __construct(\Mailster\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \Mailster\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \Mailster\Aws\Retry\RateLimiter();
        }
    }
        ...
        $callback = function ($value) use($handler, $cmd, $req, $decider, $delayer, &$attempts, &$requestStats, &$monitoringEvents, &$callback) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($attempts, $cmd, $value)) {
                    return \Mailster\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
            } elseif ($value instanceof \Mailster\Aws\ResultInterface && !$decider($attempts, $cmd, $value)) {
    ...
    private static function isRetryable($result, $retryCurlErrors, $options = [])
    {
        ...
        if (!$result instanceof \Mailster\Aws\Exception\AwsException) {
            return \false;
        }
        ...
        if (\count($retryCurlErrors) && ($previous = $result->getPrevious()) && $previous instanceof \Mailster\GuzzleHttp\Exception\RequestException) {
