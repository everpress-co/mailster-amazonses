...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'Mailster\\Aws\\Exception\\AwsException', $collectStats = \false)
    {
    ...
    public function __invoke(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \Mailster\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\Mailster\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \Mailster\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request, \Mailster\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \Mailster\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \Mailster\Psr\Http\Message\RequestInterface $request, \Mailster\Aws\CommandInterface $command, array $stats)
    {
