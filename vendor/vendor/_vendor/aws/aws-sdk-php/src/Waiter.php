 ...
 */
class Waiter implements \Mailster\GuzzleHttp\Promise\PromisorInterface
{
     ...
     */
    public function __construct(\Mailster\Aws\AwsClientInterface $client, $name, array $args = [], array $config = [])
    {
        $this->client = $client;
        $this->name = $name;
        $this->args = $args;
        ...
        return \Mailster\GuzzleHttp\Promise\coroutine(function () {
            ...
            for ($state = 'retry', $attempt = 1; $state === 'retry'; $attempt++) {
                ...
                if ($state === 'success') {
                    ...
                    (yield new \Mailster\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException($msg)));
                } elseif ($state === 'retry' && $attempt >= $this->config['maxAttempts']) {
                    $state = 'failed';
                    (yield new \Mailster\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException("The {$this->name} waiter failed after attempt #{$attempt}.")));
                }
            }
        });
    }
    ...
    {
        return !$result instanceof \Mailster\Aws\ResultInterface ? \false : $acceptor['expected'] == $result->search($acceptor['argument']);
    }
    /**
     * @param Result $result   Result or exception.
     * @param array  $acceptor Acceptor configuration being checked.
    ...
    {
        if (!$result instanceof \Mailster\Aws\ResultInterface) {
            return \false;
        }
    ...
    private function matchesStatus($result, array $acceptor)
    {
        ...
        if ($result instanceof \Mailster\Aws\Exception\AwsException && ($response = $result->getResponse())) {
