 ...
 * @internal
 */
class Arn implements \Mailster\Aws\Arn\ArnInterface
{
    protected $data;
    protected $string;
    ...
    public function __construct($data)
    {
        if (\is_array($data)) {
            ...
            throw new \Mailster\Aws\Arn\Exception\InvalidArnException('Constructor accepts a string or an' . ' array as an argument.');
        }
    ...
    {
        if ($data['arn'] !== 'arn') {
            throw new \Mailster\Aws\Arn\Exception\InvalidArnException("The 1st component of an ARN must be" . " 'arn'.");
        }
        if (empty($data['partition'])) {
            throw new \Mailster\Aws\Arn\Exception\InvalidArnException("The 2nd component of an ARN" . " represents the partition and must not be empty.");
        }
        if (empty($data['service'])) {
            throw new \Mailster\Aws\Arn\Exception\InvalidArnException("The 3rd component of an ARN" . " represents the service and must not be empty.");
        }
        if (empty($data['resource'])) {
            throw new \Mailster\Aws\Arn\Exception\InvalidArnException("The 6th component of an ARN" . " represents the resource information and must not be empty." . " Individual service ARNs may include additional delimiters" . " to further qualify resources.");
        }
    }
}
