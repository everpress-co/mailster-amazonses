...
namespace Mailster\Aws\Sqs;

use Mailster\Aws\AwsClient;
use Mailster\Aws\CommandInterface;
use Mailster\Aws\Sqs\Exception\SqsException;
 ...
 */
class SqsClient extends \Mailster\Aws\AwsClient
{
        ...
        return static function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $c, \Mailster\Psr\Http\Message\RequestInterface $r = null) use($handler) {
                if ($c->hasParam('QueueUrl')) {
                    $r = $r->withUri(\Mailster\GuzzleHttp\Psr7\UriResolver::resolve($r->getUri(), new \Mailster\GuzzleHttp\Psr7\Uri($c['QueueUrl'])));
                }
                return $handler($c, $r);
            };
        };
                ...
                return $handler($c, $r)->then(function ($result) use($c, $r) {
                    foreach ((array) $result['Messages'] as $msg) {
                        ...
                        if (isset($msg['MD5OfBody']) && $bodyMd5 !== $msg['MD5OfBody']) {
                            throw new \Mailster\Aws\Sqs\Exception\SqsException(\sprintf('MD5 mismatch. Expected %s, found %s', $msg['MD5OfBody'], $bodyMd5), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                        }
                        if (isset($msg['MD5OfMessageAttributes'])) {
                            ...
                            if ($messageAttributesMd5 !== $msg['MD5OfMessageAttributes']) {
                                throw new \Mailster\Aws\Sqs\Exception\SqsException(\sprintf('Attribute MD5 mismatch. Expected %s, found %s', $msg['MD5OfMessageAttributes'], $messageAttributesMd5 ? $messageAttributesMd5 : 'No Attributes'), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                            }
                        } else {
                            if (isset($msg['MessageAttributes'])) {
                                throw new \Mailster\Aws\Sqs\Exception\SqsException(\sprintf('No Attribute MD5 found. Expected %s', self::calculateMessageAttributesMd5($msg)), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                            }
                        }
                    }
                    return $result;
