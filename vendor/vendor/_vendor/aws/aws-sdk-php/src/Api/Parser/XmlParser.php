...
class XmlParser
{
    public function parse(\Mailster\Aws\Api\StructureShape $shape, \SimpleXMLElement $value)
    {
    ...
    private function parse_structure(\Mailster\Aws\Api\StructureShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
    ...
    private function memberKey(\Mailster\Aws\Api\Shape $shape, $name)
    {
        if (null !== $shape['locationName']) {
            return $shape['locationName'];
        }
        if ($shape instanceof \Mailster\Aws\Api\ListShape && $shape['flattened']) {
            return $shape->getMember()['locationName'] ?: $name;
        }
        return $name;
    }
    private function parse_list(\Mailster\Aws\Api\ListShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
    ...
    private function parse_map(\Mailster\Aws\Api\MapShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
        if (!$shape['flattened']) {
            $value = $value->entry;
    ...
    private function parse_blob(\Mailster\Aws\Api\Shape $shape, $value)
    {
        return \base64_decode((string) $value);
    }
    private function parse_float(\Mailster\Aws\Api\Shape $shape, $value)
    {
        return (float) (string) $value;
    }
    private function parse_integer(\Mailster\Aws\Api\Shape $shape, $value)
    {
        return (int) (string) $value;
    }
    private function parse_boolean(\Mailster\Aws\Api\Shape $shape, $value)
    {
        return $value == 'true';
    }
    private function parse_timestamp(\Mailster\Aws\Api\Shape $shape, $value)
    {
        if (!empty($shape['timestampFormat']) && $shape['timestampFormat'] === 'unixTimestamp') {
            return \Mailster\Aws\Api\DateTimeResult::fromEpoch((string) $value);
        }
        return new \Mailster\Aws\Api\DateTimeResult($value);
    }
    private function parse_xml_attribute(\Mailster\Aws\Api\Shape $shape, \Mailster\Aws\Api\Shape $memberShape, $value)
    {
