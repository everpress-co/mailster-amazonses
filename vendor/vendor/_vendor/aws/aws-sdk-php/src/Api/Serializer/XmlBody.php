...
class XmlBody
{
     ...
     * @return string
     */
    public function build(\Mailster\Aws\Api\Shape $shape, array $args)
    {
        $xml = new \XMLWriter();
        $xml->openMemory();
    ...
    private function startElement(\Mailster\Aws\Api\Shape $shape, $name, \XMLWriter $xml)
    {
    ...
    private function format(\Mailster\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
    ...
    private function defaultShape(\Mailster\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->text($value);
        $xml->endElement();
    }
    private function add_structure(\Mailster\Aws\Api\StructureShape $shape, $name, array $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
    ...
    private function getStructureMembers(\Mailster\Aws\Api\StructureShape $shape, array $value)
    {
        $members = [];
    ...
    private function add_list(\Mailster\Aws\Api\ListShape $shape, $name, array $value, \XMLWriter $xml)
    {
    ...
    private function add_map(\Mailster\Aws\Api\MapShape $shape, $name, array $value, \XMLWriter $xml)
    {
    ...
    private function add_blob(\Mailster\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->writeRaw(\base64_encode($value));
        $xml->endElement();
    }
    private function add_timestamp(\Mailster\Aws\Api\TimestampShape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $xml->writeRaw(\Mailster\Aws\Api\TimestampShape::format($value, $timestampFormat));
        $xml->endElement();
    }
    private function add_boolean(\Mailster\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->writeRaw($value ? 'true' : 'false');
        $xml->endElement();
    }
    private function add_string(\Mailster\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
