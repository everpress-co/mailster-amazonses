...
use Mailster\Aws\Api\Serializer\QuerySerializer;
use Mailster\Aws\Api\Serializer\Ec2ParamBuilder;
use Mailster\Aws\Api\Parser\QueryParser;
/**
 * Represents a web service API model.
 */
class Service extends \Mailster\Aws\Api\AbstractModel
{
    /** @var callable */
    private $apiProvider;
    /** @var string */
    ...
    public function __construct(array $definition, callable $provider)
    {
        ...
        parent::__construct($definition, new \Mailster\Aws\Api\ShapeMap($definition['shapes']));
     ...
     */
    public static function createSerializer(\Mailster\Aws\Api\Service $api, $endpoint)
    {
        static $mapping = ['json' => 'Mailster\\Aws\\Api\\Serializer\\JsonRpcSerializer', 'query' => 'Mailster\\Aws\\Api\\Serializer\\QuerySerializer', 'rest-json' => 'Mailster\\Aws\\Api\\Serializer\\RestJsonSerializer', 'rest-xml' => 'Mailster\\Aws\\Api\\Serializer\\RestXmlSerializer'];
        $proto = $api->getProtocol();
        if (isset($mapping[$proto])) {
            return new $mapping[$proto]($api, $endpoint);
        }
        if ($proto == 'ec2') {
            return new \Mailster\Aws\Api\Serializer\QuerySerializer($api, $endpoint, new \Mailster\Aws\Api\Serializer\Ec2ParamBuilder());
        }
     ...
     * @throws \UnexpectedValueException
     */
    public static function createErrorParser($protocol, \Mailster\Aws\Api\Service $api = null)
    {
        static $mapping = ['json' => 'Mailster\\Aws\\Api\\ErrorParser\\JsonRpcErrorParser', 'query' => 'Mailster\\Aws\\Api\\ErrorParser\\XmlErrorParser', 'rest-json' => 'Mailster\\Aws\\Api\\ErrorParser\\RestJsonErrorParser', 'rest-xml' => 'Mailster\\Aws\\Api\\ErrorParser\\XmlErrorParser', 'ec2' => 'Mailster\\Aws\\Api\\ErrorParser\\XmlErrorParser'];
     ...
     * @throws \UnexpectedValueException
     */
    public static function createParser(\Mailster\Aws\Api\Service $api)
    {
        static $mapping = ['json' => 'Mailster\\Aws\\Api\\Parser\\JsonRpcParser', 'query' => 'Mailster\\Aws\\Api\\Parser\\QueryParser', 'rest-json' => 'Mailster\\Aws\\Api\\Parser\\RestJsonParser', 'rest-xml' => 'Mailster\\Aws\\Api\\Parser\\RestXmlParser'];
        $proto = $api->getProtocol();
        if (isset($mapping[$proto])) {
            return new $mapping[$proto]($api);
        }
        if ($proto == 'ec2') {
            return new \Mailster\Aws\Api\Parser\QueryParser($api, null, \false);
        }
        throw new \UnexpectedValueException('Unknown protocol: ' . $api->getProtocol());
    }
    /**
    ...
    public function getOperation($name)
    {
        if (!isset($this->operations[$name])) {
            ...
            $this->operations[$name] = new \Mailster\Aws\Api\Operation($this->definition['operations'][$name], $this->shapeMap);
        }
    ...
    public function getErrorShapes()
    {
        ...
        foreach ($this->definition['shapes'] as $name => $definition) {
            if (!empty($definition['exception'])) {
                ...
                $result[] = new \Mailster\Aws\Api\StructureShape($definition, $this->getShapeMap());
            }
        }
        return $result;
    }
