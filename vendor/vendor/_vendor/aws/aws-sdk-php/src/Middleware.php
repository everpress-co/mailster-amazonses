...
final class Middleware
{
     ...
     * @return callable
     */
    public static function sourceFile(\Mailster\Aws\Api\Service $api, $bodyParameter = 'Body', $sourceParameter = 'SourceFile')
    {
        return function (callable $handler) use($api, $bodyParameter, $sourceParameter) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler, $api, $bodyParameter, $sourceParameter) {
                $operation = $api->getOperation($command->getName());
                $source = $command[$sourceParameter];
                if ($source !== null && $operation->getInput()->hasMember($bodyParameter)) {
                    $command[$bodyParameter] = new \Mailster\GuzzleHttp\Psr7\LazyOpenStream($source, 'r');
     ...
     * @return callable
     */
    public static function validation(\Mailster\Aws\Api\Service $api, \Mailster\Aws\Api\Validator $validator = null)
    {
        $validator = $validator ?: new \Mailster\Aws\Api\Validator();
        return function (callable $handler) use($api, $validator) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($api, $validator, $handler) {
                $operation = $api->getOperation($command->getName());
    ...
    public static function requestBuilder(callable $serializer)
    {
        return function (callable $handler) use($serializer) {
            return function (\Mailster\Aws\CommandInterface $command) use($serializer, $handler) {
    ...
    public static function signer(callable $credProvider, callable $signatureFunction)
    {
        return function (callable $handler) use($signatureFunction, $credProvider) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request) use($handler, $signatureFunction, $credProvider) {
                $signer = $signatureFunction($command);
                return $credProvider()->then(function (\Mailster\Aws\Credentials\CredentialsInterface $creds) use($handler, $command, $signer, $request) {
    ...
    public static function tap(callable $fn)
    {
        return function (callable $handler) use($fn) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler, $fn) {
                $fn($command, $request);
                return $handler($command, $request);
            };
        };
    ...
    {
        $decider = $decider ?: \Mailster\Aws\RetryMiddleware::createDefaultDecider();
        $delay = $delay ?: [\Mailster\Aws\RetryMiddleware::class, 'exponentialDelay'];
        return function (callable $handler) use($decider, $delay, $stats) {
            return new \Mailster\Aws\RetryMiddleware($decider, $delay, $handler, $stats);
        };
    }
    /**
    ...
    public static function invocationId()
    {
        return function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request) use($handler) {
    ...
    public static function contentType(array $operations)
    {
        return function (callable $handler) use($operations) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler, $operations) {
                if (!$request->hasHeader('Content-Type') && \in_array($command->getName(), $operations, \true) && ($uri = $request->getBody()->getMetadata('uri'))) {
                    $request = $request->withHeader('Content-Type', \Mailster\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream');
                }
                return $handler($command, $request);
            };
        };
     ...
     * @return callable
     */
    public static function history(\Mailster\Aws\History $history)
    {
        return function (callable $handler) use($history) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler, $history) {
                ...
                return $handler($command, $request)->then(function ($result) use($history, $ticket) {
                    ...
                    return \Mailster\GuzzleHttp\Promise\rejection_for($reason);
                });
            };
        };
    }
    ...
    public static function mapRequest(callable $f)
    {
        return function (callable $handler) use($f) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler, $f) {
    ...
    public static function timer()
    {
        return function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                $start = \microtime(\true);
                return $handler($command, $request)->then(function (\Mailster\Aws\ResultInterface $res) use($start) {
                    ...
                    if ($err instanceof \Mailster\Aws\Exception\AwsException) {
                        $err->setTransferInfo(['total_time' => \microtime(\true) - $start] + $err->getTransferInfo());
                    }
                    return \Mailster\GuzzleHttp\Promise\rejection_for($err);
                });
            };
        };
    }
