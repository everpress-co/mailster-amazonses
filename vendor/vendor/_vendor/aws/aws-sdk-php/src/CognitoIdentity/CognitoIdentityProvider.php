...
namespace Mailster\Aws\CognitoIdentity;
...
class CognitoIdentityProvider
{
    ...
    public function __construct($poolId, array $clientOptions, array $logins = [], $accountId = null)
    {
        ...
        $this->client = new \Mailster\Aws\CognitoIdentity\CognitoIdentityClient($clientOptions + ['credentials' => \false]);
    }
    public function __invoke()
    {
        return \Mailster\GuzzleHttp\Promise\coroutine(function () {
            ...
            (yield new \Mailster\Aws\Credentials\Credentials($result['Credentials']['AccessKeyId'], $result['Credentials']['SecretKey'], $result['Credentials']['SessionToken'], (int) $result['Credentials']['Expiration']->format('U')));
        });
    }
