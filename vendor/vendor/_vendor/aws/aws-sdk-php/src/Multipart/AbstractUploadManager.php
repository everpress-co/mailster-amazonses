...
namespace Mailster\Aws\Multipart;

use Mailster\Aws\AwsClientInterface as Client;
use Mailster\Aws\CommandInterface;
use Mailster\Aws\CommandPool;
use Mailster\Aws\Exception\AwsException;
use Mailster\Aws\Exception\MultipartUploadException;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \Mailster\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'Mailster\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\Mailster\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \Mailster\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\Mailster\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \Mailster\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\Mailster\Aws\Multipart\UploadState::COMPLETED);
        })->otherwise($this->buildFailureCatch());
    }
    private function transformException($e)
    {
        // Throw errors from the operations as a specific Multipart error.
        if ($e instanceof \Mailster\Aws\Exception\AwsException) {
     ...
     */
    protected abstract function handleResult(\Mailster\Aws\CommandInterface $command, \Mailster\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        if ($this->config['state'] instanceof \Mailster\Aws\Multipart\UploadState) {
        ...
        $state = new \Mailster\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\Mailster\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\Mailster\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \Mailster\Aws\Result();
                });
            };
        };
    }
