...
abstract class AbstractUploader extends \Mailster\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\Mailster\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\Mailster\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \Mailster\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
    ...
    private function determineSource($source)
    {
        ...
        if (\is_string($source)) {
            $source = \Mailster\GuzzleHttp\Psr7\try_fopen($source, 'r');
        }
        // Create a source stream.
        $stream = \Mailster\GuzzleHttp\Psr7\stream_for($source);
