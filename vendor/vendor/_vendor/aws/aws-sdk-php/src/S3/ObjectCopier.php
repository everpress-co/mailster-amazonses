 ...
 * when appropriate.
 */
class ObjectCopier implements \Mailster\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \Mailster\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\Mailster\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \Mailster\GuzzleHttp\Promise\coroutine(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \Mailster\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
    ...
    {
        if (\Mailster\Aws\Arn\ArnParser::isArn($this->source['Bucket'])) {
            try {
                new \Mailster\Aws\Arn\S3\AccessPointArn($this->source['Bucket']);
            } catch (\Exception $e) {
