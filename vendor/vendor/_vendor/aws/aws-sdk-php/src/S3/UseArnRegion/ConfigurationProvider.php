...
class ConfigurationProvider extends \Mailster\Aws\AbstractConfigurationProvider implements \Mailster\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \Mailster\Aws\S3\UseArnRegion\ConfigurationInterface::class;
    protected static $exceptionClass = \Mailster\Aws\S3\UseArnRegion\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['use_arn_region']) && $config['use_arn_region'] instanceof \Mailster\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($useArnRegion)) {
                return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\S3\UseArnRegion\Configuration($useArnRegion));
            }
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \Mailster\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
            if ($data === \false) {
            ...
            return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\S3\UseArnRegion\Configuration($data[$profile][self::INI_USE_ARN_REGION]));
        };
    }
    /**
    ...
    {
        return function () {
            return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\S3\UseArnRegion\Configuration(self::DEFAULT_USE_ARN_REGION));
        };
    }
}
