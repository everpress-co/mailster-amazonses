...
class S3EndpointMiddleware
{
    ...
    public function __invoke(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private static function isRequestHostStyleCompatible(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        return \Mailster\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']) && ($request->getUri()->getScheme() === 'http' || \strpos($command['Bucket'], '.') === \false) && \filter_var($request->getUri()->getHost(), \FILTER_VALIDATE_IP) === \false;
    }
    private function endpointPatternDecider(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function canAccelerate(\Mailster\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \Mailster\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\Mailster\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyDualStackEndpoint(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\Mailster\Aws\CommandInterface $command, $pattern)
    {
        return "{$command['Bucket']}.{$pattern}.amazonaws.com";
    }
    private function getBucketlessPath($path, \Mailster\Aws\CommandInterface $command)
    {
