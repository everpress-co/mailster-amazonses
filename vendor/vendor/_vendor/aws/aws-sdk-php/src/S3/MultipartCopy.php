...
use Mailster\Aws\Multipart\AbstractUploadManager;
use Mailster\Aws\ResultInterface;
use Mailster\GuzzleHttp\Psr7;
class MultipartCopy extends \Mailster\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if (\Mailster\Aws\Arn\ArnParser::isArn($source)) {
            $this->source = '';
        } else {
            $this->source = "/";
        }
    ...
    {
        if ($this->config['source_metadata'] instanceof \Mailster\Aws\ResultInterface) {
        ...
        if (\strpos($key, '?')) {
            ...
            $headParams['Key'] = $key;
            $query = \Mailster\GuzzleHttp\Psr7\parse_query($query, \false);
