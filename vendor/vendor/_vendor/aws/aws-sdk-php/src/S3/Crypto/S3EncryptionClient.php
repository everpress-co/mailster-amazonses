...
use Mailster\Aws\Crypto\AbstractCryptoClient;
use Mailster\Aws\Crypto\EncryptionTrait;
use Mailster\Aws\Crypto\DecryptionTrait;
use Mailster\Aws\Crypto\MetadataEnvelope;
use Mailster\Aws\Crypto\MaterialsProvider;
use Mailster\Aws\Crypto\Cipher\CipherBuilderTrait;
use Mailster\Aws\S3\S3Client;
 ...
 */
class S3EncryptionClient extends \Mailster\Aws\Crypto\AbstractCryptoClient
{
     ...
     */
    public function __construct(\Mailster\Aws\S3\S3Client $client, $instructionFileSuffix = null)
    {
        $this->client = $client;
    ...
    public function putObjectAsync(array $args)
    {
        ...
        $envelope = new \Mailster\Aws\Crypto\MetadataEnvelope();
        return \Mailster\GuzzleHttp\Promise\promise_for($this->encrypt(\Mailster\GuzzleHttp\Psr7\stream_for($args['Body']), $args['@CipherOptions'] ?: [], $provider, $envelope))->then(function ($encryptedBodyStream) use($args) {
            $hash = new \Mailster\Aws\PhpHash('sha256');
            $hashingEncryptedBodyStream = new \Mailster\Aws\HashingStream($encryptedBodyStream, $hash, self::getContentShaDecorator($args));
