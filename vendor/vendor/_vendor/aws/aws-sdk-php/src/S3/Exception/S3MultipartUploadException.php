...
namespace Mailster\Aws\S3\Exception;

use Mailster\Aws\CommandInterface;
use Mailster\Aws\Exception\AwsException;
use Mailster\Aws\Multipart\UploadState;
class S3MultipartUploadException extends \Mailster\Aws\Exception\MultipartUploadException
{
     ...
     */
    public function __construct(\Mailster\Aws\Multipart\UploadState $state, $prev = null)
    {
        if (\is_array($prev) && ($error = $prev[\key($prev)])) {
            $this->collectPathInfo($error->getCommand());
        } elseif ($prev instanceof \Mailster\Aws\Exception\AwsException) {
     ...
     */
    private function collectPathInfo(\Mailster\Aws\CommandInterface $cmd)
    {
