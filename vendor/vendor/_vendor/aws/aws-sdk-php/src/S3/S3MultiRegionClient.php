...
use Mailster\Aws\MultiRegionClient as BaseClient;
use Mailster\Aws\Exception\AwsException;
use Mailster\Aws\S3\Exception\PermanentRedirectException;
 ...
 */
class S3MultiRegionClient extends \Mailster\Aws\MultiRegionClient implements \Mailster\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\Mailster\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \Mailster\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \Mailster\GuzzleHttp\Promise\coroutine(function () use($handler, $command, $cacheKey) {
                    try {
                        (yield $handler($command));
                    } catch (\Mailster\Aws\S3\Exception\PermanentRedirectException $e) {
                        ...
                        $command['@region'] = $region;
                        (yield $handler($command));
                    } catch (\Mailster\Aws\Exception\AwsException $e) {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \Mailster\GuzzleHttp\Promise\promise_for($cached);
        }
