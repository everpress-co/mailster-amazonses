...
use Mailster\Aws\Signature\SignatureTrait;
use Mailster\Aws\Signature\SignatureV4 as SignatureV4;
use Mailster\Aws\Api\TimestampShape as TimestampShape;
/**
...
class PostObjectV4
{
     ...
     */
    public function __construct(\Mailster\Aws\S3\S3ClientInterface $client, $bucket, array $formInputs, array $options = [], $expiration = '+1 hours')
    {
        $this->client = $client;
        $this->bucket = $bucket;
        ...
        // setup basic policy
        $policy = ['expiration' => \Mailster\Aws\Api\TimestampShape::format($expiration, 'iso8601'), 'conditions' => $options];
    ...
    {
        $uri = new \Mailster\GuzzleHttp\Psr7\Uri($this->client->getEndpoint());
    ...
    protected function getPolicyAndSignature(\Mailster\Aws\Credentials\CredentialsInterface $credentials, array $policy)
    {
        $ldt = \gmdate(\Mailster\Aws\Signature\SignatureV4::ISO8601_BASIC);
        $sdt = \substr($ldt, 0, 8);
