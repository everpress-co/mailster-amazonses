...
class S3UriParser
{
    ...
    public function parse($uri)
    {
        ...
        if (!empty($components)) {
            if (\Mailster\Aws\Arn\ArnParser::isArn($components['host'])) {
                $arn = new \Mailster\Aws\Arn\S3\AccessPointArn($components['host']);
                return ['bucket' => $components['host'], 'key' => $components['path'], 'path_style' => \false, 'region' => $arn->getRegion()];
            }
        }
        $url = \Mailster\GuzzleHttp\Psr7\uri_for($uri);
    ...
    private function parseStreamWrapper(\Mailster\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\Mailster\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\Mailster\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = \ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\Mailster\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = \false;
