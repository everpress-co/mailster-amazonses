...
use Mailster\Aws\S3\Exception\S3MultipartUploadException;
/**
...
class MultipartUploader extends \Mailster\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\Mailster\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, \array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \Mailster\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \Mailster\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
    ...
    protected function extractETag(\Mailster\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \Mailster\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream';
        }
    }
    ...
    private function decorateWithHashes(\Mailster\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        ...
        $hash = new \Mailster\Aws\PhpHash('sha256');
        return new \Mailster\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
