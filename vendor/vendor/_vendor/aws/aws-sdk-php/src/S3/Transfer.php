...
use Mailster\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
/**
 ...
 */
class Transfer implements \Mailster\GuzzleHttp\Promise\PromisorInterface
{
    private $client;
    private $promise;
    private $source;
     ...
     */
    public function __construct(\Mailster\Aws\S3\S3ClientInterface $client, $source, $dest, array $options = [])
    {
        $this->client = $client;
        ...
        $this->concurrency = isset($options['concurrency']) ? $options['concurrency'] : \Mailster\Aws\S3\MultipartUploader::DEFAULT_CONCURRENCY;
        $this->mupThreshold = isset($options['mup_threshold']) ? $options['mup_threshold'] : 16777216;
        if ($this->mupThreshold < \Mailster\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
    ...
    private function createDownloadPromise()
    {
        ...
        foreach ($this->getDownloadsIterator() as $object) {
            ...
            if (\strpos($this->resolveUri($resolveSink), $this->destination['path']) !== 0) {
                throw new \Mailster\Aws\Exception\AwsException('Cannot download key ' . $objectKey . ', its relative path resolves outside the' . ' parent directory', $command);
            }
        ...
        return (new \Mailster\Aws\CommandPool($this->client, $commands, ['concurrency' => $this->concurrency, 'before' => $this->before, 'rejected' => function ($reason, $idx, \Mailster\GuzzleHttp\Promise\PromiseInterface $p) {
            $p->reject($reason);
        }]))->promise();
    }
    private function createUploadPromise()
    {
        ...
        $files = \Mailster\Aws\map($this->getUploadsIterator(), function ($file) {
        ...
        return \Mailster\GuzzleHttp\Promise\each_limit_all($files, $this->concurrency);
    }
    /** @return Iterator */
    private function getUploadsIterator()
    {
        if (\is_string($this->source)) {
            return \Mailster\Aws\filter(\Mailster\Aws\recursive_dir_iterator($this->sourceMetadata['path']), function ($file) {
                return !\is_dir($file);
            });
        }
        return $this->source;
    ...
    private function getDownloadsIterator()
    {
        if (\is_string($this->source)) {
            ...
            $files = \Mailster\Aws\map($files, function ($key) use($listArgs) {
                return "s3://{$listArgs['Bucket']}/{$key}";
            });
            return \Mailster\Aws\filter($files, function ($key) {
    ...
    private function uploadMultipart($filename)
    {
        $args = $this->s3Args;
        $args['Key'] = $this->createS3Key($filename);
        return (new \Mailster\Aws\S3\MultipartUploader($this->client, $filename, ['bucket' => $args['Bucket'], 'key' => $args['Key'], 'before_initiate' => $this->before, 'before_upload' => $this->before, 'before_complete' => $this->before, 'concurrency' => $this->concurrency]))->promise();
    }
    ...
    private function addDebugToBefore($debug)
    {
        ...
        $s3Args = $this->s3Args;
        $this->before = static function (\Mailster\Aws\CommandInterface $command) use($before, $debug, $sourcePath, $s3Args) {
