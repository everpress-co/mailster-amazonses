...
use Mailster\Aws\Arn\ArnParser;
use Mailster\Aws\Arn\Exception\InvalidArnException;
use Mailster\Aws\Arn\S3\AccessPointArn;
use Mailster\Aws\CommandInterface;
use Mailster\Aws\Endpoint\PartitionEndpointProvider;
use Mailster\Aws\Exception\InvalidRegionException;
use Mailster\Aws\Exception\UnresolvedEndpointException;
use Mailster\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\Mailster\Aws\Api\Service $service, $region, array $config)
    {
        return function (callable $handler) use($service, $region, $config) {
            return new self($handler, $service, $region, $config);
        };
    }
    public function __construct(callable $nextHandler, \Mailster\Aws\Api\Service $service, $region, array $config = [])
    {
        $this->partitionProvider = \Mailster\Aws\Endpoint\PartitionEndpointProvider::defaultProvider();
        $this->region = $region;
        $this->service = $service;
        $this->config = $config;
        $this->nextHandler = $nextHandler;
    }
    public function __invoke(\Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $req)
    {
        ...
        if (!empty($op['input']['shape'])) {
            ...
            if (!empty($input = $service['shapes'][$op['input']['shape']])) {
                ...
                if (!empty($arnableKey) && \Mailster\Aws\Arn\ArnParser::isArn($cmd[$arnableKey])) {
                    try {
                        // Throw for commands that do not support ARN inputs
                        if (\in_array($cmd->getName(), $this->nonArnableCommands)) {
                            throw new \Mailster\Aws\S3\Exception\S3Exception('ARN values cannot be used in the bucket field for' . ' the ' . $cmd->getName() . ' operation.', $cmd);
                        }
                        $arn = \Mailster\Aws\Arn\ArnParser::parse($cmd[$arnableKey]);
                    ...
                    } catch (\Mailster\Aws\Arn\Exception\InvalidArnException $e) {
                        // Add context to ARN exception
                        throw new \Mailster\Aws\S3\Exception\S3Exception('Bucket parameter parsed as ARN and failed with: ' . $e->getMessage(), $cmd, [], $e);
                    }
                }
            }
        }
    ...
    private function generateAccessPointHost(\Mailster\Aws\Arn\S3\AccessPointArn $arn, \Mailster\Psr\Http\Message\RequestInterface $req)
    {
    ...
    private function getPartitionSuffix(\Mailster\Aws\Arn\ArnInterface $arn)
    {
    ...
    {
        $partition = \Mailster\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, 's3');
    ...
    {
        if ($arn instanceof \Mailster\Aws\Arn\S3\AccessPointArn) {
            // Accelerate is not supported with access points
            if (!empty($this->config['accelerate'])) {
                throw new \Mailster\Aws\Exception\UnresolvedEndpointException('Accelerate is currently not supported with access points.' . ' Please disable accelerate or do not supply an access' . ' point ARN.');
            }
            // Path-style is not supported with access points
            if (!empty($this->config['path_style'])) {
                throw new \Mailster\Aws\Exception\UnresolvedEndpointException('Path-style addressing is currently not supported with' . ' access points. Please disable path-style or do not' . ' supply an access point ARN.');
            }
            // Custom endpoint is not supported with access points
            if (!\is_null($this->config['endpoint'])) {
                throw new \Mailster\Aws\Exception\UnresolvedEndpointException('A custom endpoint has been supplied along with an access' . ' point ARN, and these are not compatible with each other.' . ' Please only use one or the other.');
            }
            ...
            if ($arn->getPartition() !== $clientPart->getName()) {
                throw new \Mailster\Aws\Exception\InvalidRegionException('The supplied ARN partition' . " does not match the client's partition.");
            }
            if ($clientPart->getName() !== $arnPart->getName()) {
                throw new \Mailster\Aws\Exception\InvalidRegionException('The corresponding partition' . ' for the supplied ARN region does not match the' . " client's partition.");
            }
            ...
            if (!$this->isMatchingSigningRegion($arn->getRegion(), $this->region)) {
                if (empty($this->config['use_arn_region']) || !$this->config['use_arn_region']->isUseArnRegion()) {
                    throw new \Mailster\Aws\Exception\InvalidRegionException('The region' . " specified in the ARN (" . $arn->getRegion() . ") does not match the client region (" . "{$this->region}).");
                }
            }
            return $arnPart;
        }
        throw new \Mailster\Aws\Arn\Exception\InvalidArnException('Provided ARN was not' . ' a valid S3 access point ARN');
    }
}
