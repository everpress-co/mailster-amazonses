...
namespace Mailster\Aws\Endpoint;

use ArrayAccess;
use Mailster\Aws\HasDataTrait;
use Mailster\Aws\Sts\RegionalEndpoints\ConfigurationProvider;
use Mailster\Aws\S3\RegionalEndpoint\ConfigurationProvider as S3ConfigurationProvider;
use InvalidArgumentException as Iae;
/**
 * Default implementation of an AWS partition.
 */
final class Partition implements \ArrayAccess, \Mailster\Aws\Endpoint\PartitionInterface
{
    use HasDataTrait;
    ...
    {
        return $service === 'sts' && \in_array($region, $this->stsLegacyGlobalRegions) && (empty($options['sts_regional_endpoints']) || \Mailster\Aws\Sts\RegionalEndpoints\ConfigurationProvider::unwrap($options['sts_regional_endpoints'])->getEndpointsType() !== 'regional');
    }
    /**
    ...
    {
        return $service === 's3' && $region === 'us-east-1' && (empty($options['s3_us_east_1_regional_endpoint']) || \Mailster\Aws\S3\RegionalEndpoint\ConfigurationProvider::unwrap($options['s3_us_east_1_regional_endpoint'])->getEndpointsType() !== 'regional');
    }
