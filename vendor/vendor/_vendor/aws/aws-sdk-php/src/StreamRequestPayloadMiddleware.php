...
use Mailster\Aws\Exception\IncalculablePayloadException;
...
class StreamRequestPayloadMiddleware
{
     ...
     * @return \Closure
     */
    public static function wrap(\Mailster\Aws\Api\Service $service)
    {
        return function (callable $handler) use($service) {
            return new self($handler, $service);
        };
    }
    public function __construct(callable $nextHandler, \Mailster\Aws\Api\Service $service)
    {
        ...
        if ($hasStreaming) {
            ...
            if (empty($requiresLength) && empty($contentLength) && isset($operation['authtype']) && $operation['authtype'] == 'v4-unsigned-body') {
                ...
                if (empty($contentLength)) {
                    ...
                    if (\is_null($size)) {
                        throw new \Mailster\Aws\Exception\IncalculablePayloadException('Payload' . ' content length is required and can not be' . ' calculated.');
                    }
