...
use Mailster\Aws\ResponseContainerInterface;
 ...
 * @internal
 */
abstract class AbstractMonitoringMiddleware implements \Mailster\Aws\ClientSideMonitoring\MonitoringMiddlewareInterface
{
    ...
    protected $region;
    protected $service;
    protected static function getAwsExceptionHeader(\Mailster\Aws\Exception\AwsException $e, $headerName)
    {
    ...
    protected static function getResultHeader(\Mailster\Aws\ResultInterface $result, $headerName)
    {
    ...
    {
        if ($e instanceof \Mailster\Aws\ResponseContainerInterface) {
            $response = $e->getResponse();
            if ($response instanceof \Mailster\Psr\Http\Message\ResponseInterface) {
                $header = $response->getHeader($headerName);
                if (!empty($header[0])) {
                    return $header[0];
                }
    ...
    public function __invoke(\Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $g = function ($value) use($eventData, $enabled) {
            if ($enabled) {
                ...
                if ($value instanceof \Mailster\Aws\MonitoringEventsInterface) {
                    $value->appendMonitoringEvent($eventData);
                }
            }
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                return \Mailster\GuzzleHttp\Promise\rejection_for($value);
            }
            return $value;
        };
        return \Mailster\GuzzleHttp\Promise\promise_for($handler($cmd, $request))->then($g, $g);
    }
    private function getClientId()
    {
        return $this->unwrappedOptions()->getClientId();
    }
    private function getNewEvent(\Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) \floor(\microtime(\true) * 1000), 'UserAgent' => \substr($request->getHeaderLine('User-Agent') . ' ' . \Mailster\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
     ...
     * @return array
     */
    protected function populateRequestEventData(\Mailster\Aws\CommandInterface $cmd, \Mailster\Psr\Http\Message\RequestInterface $request, array $event)
    {
    ...
    {
        if (!$this->options instanceof \Mailster\Aws\ClientSideMonitoring\ConfigurationInterface) {
            try {
                $this->options = \Mailster\Aws\ClientSideMonitoring\ConfigurationProvider::unwrap($this->options);
            } catch (\Exception $e) {
                // Errors unwrapping CSM config defaults to disabling it
                $this->options = new \Mailster\Aws\ClientSideMonitoring\Configuration(\false, \Mailster\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \Mailster\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT);
            }
        }
        return $this->options;
    }
