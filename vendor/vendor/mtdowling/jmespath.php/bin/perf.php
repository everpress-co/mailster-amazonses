...
$xdebug = new \Mailster\Composer\XdebugHandler\XdebugHandler('perf.php');
$xdebug->check();
unset($xdebug);
$dir = isset($argv[1]) ? $argv[1] : __DIR__ . '/../tests/compliance/perf';
\is_dir($dir) or die('Dir not found: ' . $dir);
// Warm up the runner
\Mailster\JmesPath\Env::search('foo', []);
$total = 0;
foreach (\glob($dir . '/*.json') as $file) {
    $total += \Mailster\runSuite($file);
}
echo "\nTotal time: {$total}\n";
function runSuite($file)
{
    ...
    foreach ($json as $suite) {
        foreach ($suite['cases'] as $case) {
            $total += \Mailster\runCase($suite['given'], $case['expression'], $case['name']);
        }
    }
    return $total;
}
...
{
    $best = 99999;
    $runtime = \Mailster\JmesPath\Env::createRuntime();
    for ($i = 0; $i < 100; $i++) {
        $t = \microtime(\true);
        ...
        if (!\getenv('CACHE')) {
            ...
            if ($runtime instanceof \Mailster\JmesPath\CompilerRuntime) {
