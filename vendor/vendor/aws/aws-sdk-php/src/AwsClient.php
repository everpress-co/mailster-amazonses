...
use Mailster\Aws\ClientSideMonitoring\ConfigurationProvider;
 ...
 */
class AwsClient implements \Mailster\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $aliases;
    ...
    {
        return \Mailster\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \Mailster\Aws\HandlerList();
        $resolver = new \Mailster\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \Mailster\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \Mailster\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'Mailster\\Aws\\Exception\\AwsException'];
        }
    ...
    private function addEndpointParameterMiddleware($args)
    {
        if (empty($args['disable_host_prefix_injection'])) {
            ...
            $list->appendBuild(\Mailster\Aws\EndpointParameterMiddleware::wrap($this->api), 'endpoint_parameter');
        }
    }
    private function addEndpointDiscoveryMiddleware($config, $args)
    {
        ...
        if (!isset($args['endpoint'])) {
            $list->appendBuild(\Mailster\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware::wrap($this, $args, $config['endpoint_discovery']), 'EndpointDiscoveryMiddleware');
        }
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\Mailster\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \Mailster\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\Mailster\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\Mailster\Aws\Middleware::invocationId(), 'invocation-id');
    }
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \Mailster\Aws\load_compiled_json($file);
    ...
    {
        $streamRequestPayloadMiddleware = \Mailster\Aws\StreamRequestPayloadMiddleware::wrap($this->api);
    ...
    {
        $aliases = \Mailster\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
