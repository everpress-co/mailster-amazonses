...
class PartitionEndpointProvider
{
    ...
    public function __construct(array $partitions, $defaultPartition = 'aws', $options = [])
    {
        $this->partitions = \array_map(function (array $definition) {
            return new \Mailster\Aws\Endpoint\Partition($definition);
    ...
    {
        $data = \Mailster\Aws\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        $prefixData = \Mailster\Aws\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
    ...
    public static function mergePrefixData($data, $prefixData)
    {
        ...
        foreach ($data["partitions"] as $index => $partition) {
            foreach ($prefixGroups as $current => $old) {
                $serviceData = \Mailster\JmesPath\Env::search("services.\"{$current}\"", $partition);
                if (!empty($serviceData)) {
                    foreach ($old as $prefix) {
                        if (empty(\Mailster\JmesPath\Env::search("services.\"{$prefix}\"", $partition))) {
