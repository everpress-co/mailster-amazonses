...
class AssumeRoleWithWebIdentityCredentialProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['client'])) {
            ...
            $this->client = new \Mailster\Aws\Sts\StsClient(['credentials' => \false, 'region' => $region, 'version' => 'latest']);
        }
    }
    /**
        ...
        return \Mailster\GuzzleHttp\Promise\coroutine(function () {
            ...
            while ($result == null) {
                try {
                    ...
                    throw new \Mailster\Aws\Exception\CredentialsException("Error reading WebIdentityTokenFile from " . $this->tokenFile, 0, $exception);
                }
                ...
                try {
                    ...
                    if ($e->getAwsErrorCode() == 'InvalidIdentityToken') {
                        if ($this->attempts < $this->retries) {
                            ...
                            throw new \Mailster\Aws\Exception\CredentialsException("InvalidIdentityToken, retries exhausted");
                        }
                    } else {
                        throw new \Mailster\Aws\Exception\CredentialsException("Error assuming role from web identity credentials", 0, $e);
                    }
                } catch (\Exception $e) {
                    throw new \Mailster\Aws\Exception\CredentialsException("Error retrieving web identity credentials: " . $e->getMessage() . " (" . $e->getCode() . ")");
                }
                $this->attempts++;
            }
