...
use Mailster\Aws\Exception\InvalidJsonException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->attempts = 0;
        $this->client = isset($config['client']) ? $config['client'] : \Mailster\Aws\default_http_handler();
    }
    /**
    ...
    {
        return \Mailster\GuzzleHttp\Promise\coroutine(function () {
            ...
            while ($this->secureMode && \is_null($token)) {
                ...
                } catch (\Mailster\GuzzleHttp\Exception\RequestException $e) {
            ...
            while ($result == null) {
                ...
                } catch (\Mailster\Aws\Exception\InvalidJsonException $e) {
            ...
            (yield new \Mailster\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], \strtotime($result['Expiration'])));
        });
    }
    /**
     * @param string $url
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (\strcasecmp($disabled, 'true') === 0) {
            throw new \Mailster\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata service access disabled'));
        }
        $fn = $this->client;
        $request = new \Mailster\GuzzleHttp\Psr7\Request($method, self::SERVER_URI . $url);
        $userAgent = 'aws-sdk-php/' . \Mailster\Aws\Sdk::VERSION;
        if (\defined('HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \Mailster\Aws\default_user_agent();
        $request = $request->withHeader('User-Agent', $userAgent);
        foreach ($headers as $key => $value) {
            $request = $request->withHeader($key, $value);
        }
        return $fn($request, ['timeout' => $this->timeout])->then(function (\Mailster\Psr\Http\Message\ResponseInterface $response) {
            return (string) $response->getBody();
        })->otherwise(function (array $reason) {
            $reason = $reason['exception'];
            if ($reason instanceof \Mailster\GuzzleHttp\Exception\RequestException) {
                throw $reason;
            }
            $msg = $reason->getMessage();
            throw new \Mailster\Aws\Exception\CredentialsException($this->createErrorMessage($msg));
        });
    }
    private function handleRetryableException(\Exception $e, $retryOptions, $message)
    {
        ...
        if ($isRetryable && $this->attempts < $this->retries) {
            ...
            throw new \Mailster\Aws\Exception\CredentialsException($message);
        }
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (\json_last_error() > 0) {
            throw new \Mailster\Aws\Exception\InvalidJsonException();
        }
        if ($result['Code'] !== 'Success') {
            throw new \Mailster\Aws\Exception\CredentialsException('Unexpected instance profile ' . 'response code: ' . $result['Code']);
        }
        return $result;
    }
}
