...
namespace Mailster\Aws\Credentials;

use Mailster\Aws;
use Mailster\Aws\Api\DateTimeResult;
use Mailster\Aws\CacheInterface;
use Mailster\Aws\Exception\CredentialsException;
use Mailster\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (!empty(\getenv(\Mailster\Aws\Credentials\EcsCredentialProvider::ENV_URI))) {
        ...
        if (isset($config['credentials']) && $config['credentials'] instanceof \Mailster\Aws\CacheInterface) {
     ...
     * @return callable
     */
    public static function fromCredentials(\Mailster\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \Mailster\GuzzleHttp\Promise\promise_for($creds);
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\Mailster\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return new \Mailster\GuzzleHttp\Promise\RejectedPromise($reason);
            });
        };
    }
    /**
     ...
     * @return callable
     */
    public static function cache(callable $provider, \Mailster\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof \Mailster\Aws\Credentials\CredentialsInterface && !$found->isExpired()) {
                return \Mailster\GuzzleHttp\Promise\promise_for($found);
            }
            return $provider()->then(function (\Mailster\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\Credentials\Credentials($key, $secret, \getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \Mailster\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    {
        return new \Mailster\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \Mailster\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \Mailster\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \Mailster\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \Mailster\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
            if ($data === \false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \Mailster\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \Mailster\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient)
    {
        ...
        if (empty($stsClient)) {
            ...
            $sourceCredentials = \call_user_func(\Mailster\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            $stsClient = new \Mailster\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $creds = $stsClient->createCredentials($result);
        return \Mailster\GuzzleHttp\Promise\promise_for($creds);
    }
    /**
    ...
    {
        $profileData = \Mailster\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && \getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \Mailster\Aws\parse_ini_file($configFilename, \true, \INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (\file_exists($credFile)) {
            $profiles = \Mailster\Aws\parse_ini_file($credFile, \true, \INI_SCANNER_RAW);
        }
        if (\file_exists($configFile)) {
            $configProfileData = \Mailster\Aws\parse_ini_file($configFile, \true, \INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = \str_replace('profile ', '', $name);
    ...
    {
        return new \Mailster\GuzzleHttp\Promise\RejectedPromise(new \Mailster\Aws\Exception\CredentialsException($msg));
    }
}
