...
class AssumeRoleCredentialProvider
{
    ...
    {
        $client = $this->client;
        return $client->assumeRoleAsync($this->assumeRoleParams)->then(function (\Mailster\Aws\Result $result) {
            return $this->client->createCredentials($result);
        })->otherwise(function (\RuntimeException $exception) {
            throw new \Mailster\Aws\Exception\CredentialsException("Error in retrieving assume role credentials.", 0, $exception);
        });
    }
}
