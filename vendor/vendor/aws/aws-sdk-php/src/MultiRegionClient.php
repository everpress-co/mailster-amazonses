...
use Mailster\Aws\Endpoint\PartitionInterface;
class MultiRegionClient implements \Mailster\Aws\AwsClientInterface
{
    use AwsClientTrait;
    ...
    {
        $args = \array_intersect_key(\Mailster\Aws\ClientResolver::getDefaultArguments(), ['service' => \true, 'region' => \true]);
        ...
        }], 'partition' => ['type' => 'config', 'valid' => ['string', \Mailster\Aws\Endpoint\PartitionInterface::class], 'doc' => 'AWS partition to connect to. Valid partitions' . ' include "aws," "aws-cn," and "aws-us-gov." Used to' . ' restrict the scope of the mapRegions method.', 'default' => function (array $args) {
            $region = isset($args['region']) ? $args['region'] : '';
            return \Mailster\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, $args['service']);
        }, 'fn' => function ($value, array &$args) {
            if (\is_string($value)) {
                $value = \Mailster\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartitionByName($value);
            }
            if (!$value instanceof \Mailster\Aws\Endpoint\PartitionInterface) {
    ...
    public function __construct(array $args = [])
    {
        ...
        $this->handlerList = new \Mailster\Aws\HandlerList(function (\Mailster\Aws\CommandInterface $command) {
        ...
        $resolver = new \Mailster\Aws\ClientResolver($argDefinitions);
