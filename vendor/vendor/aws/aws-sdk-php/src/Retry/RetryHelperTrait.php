...
trait RetryHelperTrait
{
    ...
    private function updateHttpStats($value, array &$stats)
    {
        ...
        if ($value instanceof \Mailster\Aws\Exception\AwsException) {
            $resultStats = isset($value->getTransferInfo('http')[0]) ? $value->getTransferInfo('http')[0] : [];
            $stats['http'][] = $resultStats;
        } elseif ($value instanceof \Mailster\Aws\ResultInterface) {
    ...
    {
        if ($return instanceof \Mailster\Aws\ResultInterface) {
            if (!isset($return['@metadata'])) {
                $return['@metadata'] = [];
            }
            $return['@metadata']['transferStats'] = $stats;
        } elseif ($return instanceof \Mailster\Aws\Exception\AwsException) {
