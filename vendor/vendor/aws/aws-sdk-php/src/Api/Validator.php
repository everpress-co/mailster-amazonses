...
class Validator
{
     ...
     */
    public function validate($name, \Mailster\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\Mailster\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\Mailster\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\Mailster\Aws\Api\ListShape $shape, $value)
    {
        if (!\is_array($value)) {
            $this->addError('must be an array. Found ' . \Mailster\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\Mailster\Aws\Api\MapShape $shape, $value)
    {
        if (!$this->checkAssociativeArray($value)) {
            return;
        }
    ...
    private function check_blob(\Mailster\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => \true, 'integer' => \true, 'double' => \true, 'resource' => \true];
        $type = \gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !\method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \Mailster\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\Mailster\Aws\Api\Shape $shape, $value)
    {
        if (!\is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \Mailster\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\Mailster\Aws\Api\Shape $shape, $value)
    {
        if (!\is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \Mailster\Aws\describe_type($value));
        }
    }
    private function check_string(\Mailster\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \Mailster\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \Mailster\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\Mailster\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        ...
        if (!$isAssociative) {
            $this->addError('must be an associative array. Found ' . \Mailster\Aws\describe_type($value));
            return \false;
        }
        return \true;
    }
