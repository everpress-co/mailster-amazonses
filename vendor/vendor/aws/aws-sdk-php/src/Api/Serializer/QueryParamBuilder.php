...
class QueryParamBuilder
{
    ...
    public function __invoke(\Mailster\Aws\Api\StructureShape $shape, array $params)
    {
        if (!$this->methods) {
    ...
    protected function format(\Mailster\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
    ...
    protected function format_structure(\Mailster\Aws\Api\StructureShape $shape, array $value, $prefix, &$query)
    {
        if ($prefix) {
            $prefix .= '.';
        }
    ...
    protected function format_map(\Mailster\Aws\Api\MapShape $shape, array $value, $prefix, array &$query)
    {
        $vals = $shape->getValue();
        $keys = $shape->getKey();
        if (!$this->isFlat($shape)) {
    ...
    protected function format_blob(\Mailster\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
        $query[$prefix] = \base64_encode($value);
    }
    protected function format_timestamp(\Mailster\Aws\Api\TimestampShape $shape, $value, $prefix, array &$query)
    {
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $query[$prefix] = \Mailster\Aws\Api\TimestampShape::format($value, $timestampFormat);
    }
    protected function format_boolean(\Mailster\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
