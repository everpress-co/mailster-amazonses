...
use Mailster\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \Mailster\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
    const ISO8601_BASIC = 'Mailster\\Ymd\\THis\\Z';
     ...
     */
    public static function convertPostToGet(\Mailster\Psr\Http\Message\RequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\Mailster\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \Mailster\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \Mailster\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \Mailster\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    ...
    private function createPresignedRequest(\Mailster\Psr\Http\Message\RequestInterface $request, \Mailster\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \Mailster\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\Mailster\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \Mailster\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
