...
use Mailster\Aws\Exception\UnresolvedSignatureException;
/**
 * Signature providers.
 *
...
class SignatureProvider
{
    ...
    public static function resolve(callable $provider, $version, $service, $region)
    {
        ...
        if ($result instanceof \Mailster\Aws\Signature\SignatureInterface) {
            return $result;
        }
        throw new \Mailster\Aws\Exception\UnresolvedSignatureException("Unable to resolve a signature for {$version}/{$service}/{$region}.\n" . "Valid signature versions include v4 and anonymous.");
    }
    /**
    ...
    public static function version()
    {
        return function ($version, $service, $region) {
            switch ($version) {
                case 's3v4':
                case 'v4':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \Mailster\Aws\Signature\S3SignatureV4($service, $region) : new \Mailster\Aws\Signature\SignatureV4($service, $region);
                case 'v4-unsigned-body':
                    return new \Mailster\Aws\Signature\SignatureV4($service, $region, ['unsigned-body' => 'true']);
                case 'anonymous':
                    return new \Mailster\Aws\Signature\AnonymousSignature();
                default:
                    return null;
            }
        };
