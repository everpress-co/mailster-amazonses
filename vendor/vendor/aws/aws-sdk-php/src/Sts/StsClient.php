 ...
 */
class StsClient extends \Mailster\Aws\AwsClient
{
    /**
     * {@inheritdoc}
     *
    ...
    public function __construct(array $args)
    {
        if (!isset($args['sts_regional_endpoints'])) {
            $args['sts_regional_endpoints'] = \Mailster\Aws\Sts\RegionalEndpoints\ConfigurationProvider::defaultProvider();
        } elseif ($args['sts_regional_endpoints'] instanceof \Mailster\Aws\CacheInterface) {
            $args['sts_regional_endpoints'] = \Mailster\Aws\Sts\RegionalEndpoints\ConfigurationProvider::defaultProvider($args);
        }
     ...
     */
    public function createCredentials(\Mailster\Aws\Result $result)
    {
        if (!$result->hasKey('Credentials')) {
            throw new \InvalidArgumentException('Result contains no credentials');
        }
        $c = $result['Credentials'];
        return new \Mailster\Aws\Credentials\Credentials($c['AccessKeyId'], $c['SecretAccessKey'], isset($c['SessionToken']) ? $c['SessionToken'] : null, isset($c['Expiration']) && $c['Expiration'] instanceof \DateTimeInterface ? (int) $c['Expiration']->format('U') : null);
    }
}
