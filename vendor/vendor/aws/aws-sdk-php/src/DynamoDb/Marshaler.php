...
class Marshaler
{
    ...
    {
        return new \Mailster\Aws\DynamoDb\BinaryValue($value);
    }
    /**
    ...
    {
        return new \Mailster\Aws\DynamoDb\NumberValue($value);
    }
    /**
    ...
    {
        return new \Mailster\Aws\DynamoDb\SetValue($values);
    }
    /**
    ...
    public function marshalValue($value)
    {
        ...
        if ($type === 'integer' || $type === 'double' || $value instanceof \Mailster\Aws\DynamoDb\NumberValue) {
        ...
        // Handle set values.
        if ($value instanceof \Mailster\Aws\DynamoDb\SetValue) {
            if (\count($value) === 0) {
        ...
        if (\is_resource($value) || $value instanceof \Mailster\Psr\Http\Message\StreamInterface) {
            $value = $this->binary($value);
        }
        if ($value instanceof \Mailster\Aws\DynamoDb\BinaryValue) {
    ...
    public function unmarshalValue(array $value, $mapAsObject = \false)
    {
        ...
        switch ($type) {
            ...
            case 'BS':
                ...
                return new \Mailster\Aws\DynamoDb\SetValue($value);
        }
