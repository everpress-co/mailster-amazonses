...
class LogRecordIterator implements \OuterIterator
{
     ...
     */
    public static function forTrail(\Mailster\Aws\S3\S3Client $s3Client, \Mailster\Aws\CloudTrail\CloudTrailClient $cloudTrailClient, array $options = [])
    {
        $logFileIterator = \Mailster\Aws\CloudTrail\LogFileIterator::forTrail($s3Client, $cloudTrailClient, $options);
        return new self(new \Mailster\Aws\CloudTrail\LogFileReader($s3Client), $logFileIterator);
    }
    /**
     * @param S3Client $s3Client
     ...
     * @return LogRecordIterator
     */
    public static function forBucket(\Mailster\Aws\S3\S3Client $s3Client, $s3BucketName, array $options = [])
    {
        $logFileReader = new \Mailster\Aws\CloudTrail\LogFileReader($s3Client);
        $iter = new \Mailster\Aws\CloudTrail\LogFileIterator($s3Client, $s3BucketName, $options);
     ...
     * @return LogRecordIterator
     */
    public static function forFile(\Mailster\Aws\S3\S3Client $s3Client, $s3BucketName, $s3ObjectKey)
    {
        $logFileReader = new \Mailster\Aws\CloudTrail\LogFileReader($s3Client);
     ...
     */
    public function __construct(\Mailster\Aws\CloudTrail\LogFileReader $logFileReader, \Iterator $logFileIterator)
    {
