...
class MockHandler implements \Countable
{
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof \Mailster\Aws\ResultInterface || $value instanceof \Mailster\Aws\Exception\AwsException || \is_callable($value)) {
                $this->queue[] = $value;
            } else {
    ...
    public function __invoke(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($result instanceof \Exception) {
            $result = new \Mailster\GuzzleHttp\Promise\RejectedPromise($result);
        } else {
            ...
            $result = \Mailster\GuzzleHttp\Promise\promise_for($result);
        }
