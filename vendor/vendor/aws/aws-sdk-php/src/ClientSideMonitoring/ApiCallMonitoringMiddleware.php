 ...
 * @internal
 */
class ApiCallMonitoringMiddleware extends \Mailster\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
    ...
    private static function getResultAttemptCount(\Mailster\Aws\ResultInterface $result)
    {
    ...
    {
        $attemptCount = 0;
        if ($e instanceof \Mailster\Aws\MonitoringEventsInterface) {
    ...
    {
        $data = [];
        if ($klass instanceof \Mailster\Aws\MonitoringEventsInterface) {
    ...
    {
        if ($klass instanceof \Mailster\Aws\Exception\AwsException && $klass->isMaxRetriesExceeded()) {
            return 1;
        }
        return 0;
    }
