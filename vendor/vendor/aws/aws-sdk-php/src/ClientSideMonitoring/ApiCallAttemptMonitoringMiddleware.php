 ...
 * @internal
 */
class ApiCallAttemptMonitoringMiddleware extends \Mailster\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
    ...
    {
        if ($klass instanceof \Mailster\Aws\ResultInterface) {
        ...
        if ($klass instanceof \Mailster\Aws\Exception\AwsException) {
    ...
    private static function getResultAttemptLatency(\Mailster\Aws\ResultInterface $result)
    {
    ...
    private static function getResultDestinationIp(\Mailster\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = \end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultDnsLatency(\Mailster\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = \end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultHttpStatusCode(\Mailster\Aws\ResultInterface $result)
    {
        return $result['@metadata']['statusCode'];
    }
    private static function getAwsExceptionAttemptLatency(\Mailster\Aws\Exception\AwsException $e)
    {
    ...
    private static function getAwsExceptionErrorCode(\Mailster\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorCode();
    }
    private static function getAwsExceptionMessage(\Mailster\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorMessage();
    }
    private static function getAwsExceptionDestinationIp(\Mailster\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['primary_ip'])) {
            return $attempt['primary_ip'];
    ...
    private static function getAwsExceptionDnsLatency(\Mailster\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['namelookup_time'])) {
            return (int) \floor($attempt['namelookup_time'] * 1000);
    ...
    private static function getAwsExceptionHttpStatusCode(\Mailster\Aws\Exception\AwsException $e)
    {
