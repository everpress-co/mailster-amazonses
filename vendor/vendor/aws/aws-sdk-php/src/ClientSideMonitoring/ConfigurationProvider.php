...
use Mailster\Aws\AbstractConfigurationProvider;
use Mailster\Aws\CacheInterface;
use Mailster\Aws\ClientSideMonitoring\Exception\ConfigurationException;
use Mailster\Aws\ConfigurationProviderInterface;
 ...
 * </code>
 */
class ConfigurationProvider extends \Mailster\Aws\AbstractConfigurationProvider implements \Mailster\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \Mailster\Aws\ClientSideMonitoring\ConfigurationInterface::class;
    protected static $exceptionClass = \Mailster\Aws\ClientSideMonitoring\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['csm']) && $config['csm'] instanceof \Mailster\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($enabled !== \false) {
                return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\ClientSideMonitoring\Configuration($enabled, \getenv(self::ENV_HOST) ?: self::DEFAULT_HOST, \getenv(self::ENV_PORT) ?: self::DEFAULT_PORT, \getenv(self::ENV_CLIENT_ID) ?: self::DEFAULT_CLIENT_ID));
            }
    ...
    {
        return function () {
            return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\ClientSideMonitoring\Configuration(self::DEFAULT_ENABLED, self::DEFAULT_HOST, self::DEFAULT_PORT, self::DEFAULT_CLIENT_ID));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \Mailster\Aws\parse_ini_file($filename, \true);
            if ($data === \false) {
            ...
            return \Mailster\GuzzleHttp\Promise\promise_for(new \Mailster\Aws\ClientSideMonitoring\Configuration($data[$profile]['csm_enabled'], $data[$profile]['csm_host'], $data[$profile]['csm_port'], $data[$profile]['csm_client_id']));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \Mailster\GuzzleHttp\Promise\PromiseInterface) {
            $config = $config->wait();
        }
        if ($config instanceof \Mailster\Aws\ClientSideMonitoring\ConfigurationInterface) {
            return $config;
            ...
            return new \Mailster\Aws\ClientSideMonitoring\Configuration($config['enabled'], $host, $port, $client_id);
        }
