...
namespace Mailster\Aws\MachineLearning;
 ...
 */
class MachineLearningClient extends \Mailster\Aws\AwsClient
{
        ...
        return static function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                if ($command->getName() === 'Predict') {
                    $request = $request->withUri(new \Mailster\GuzzleHttp\Psr7\Uri($command['PredictEndpoint']));
                }
