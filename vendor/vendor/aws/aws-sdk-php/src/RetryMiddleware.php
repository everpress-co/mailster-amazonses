...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3, $extraConfig = [])
    {
        ...
        return function ($retries, \Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request, \Mailster\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors, $extraConfig) {
            ...
            if ($retries >= $maxRetries) {
                if (!empty($error) && $error instanceof \Mailster\Aws\Exception\AwsException && $isRetryable) {
    ...
    private static function isRetryable($result, $error, $retryCurlErrors, $extraConfig = [])
    {
        ...
        if (!$error instanceof \Mailster\Aws\Exception\AwsException) {
            return \false;
        }
        ...
        if (\count($retryCurlErrors) && ($previous = $error->getPrevious()) && $previous instanceof \Mailster\GuzzleHttp\Exception\RequestException) {
     ...
     */
    public function __invoke(\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null)
    {
        $retries = 0;
        $requestStats = [];
        $monitoringEvents = [];
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$monitoringEvents, &$g) {
            ...
            } elseif ($value instanceof \Mailster\Aws\ResultInterface && !$decider($retries, $command, $request, $value, null)) {
