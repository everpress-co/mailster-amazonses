...
class ResultPaginator implements \Iterator
{
     ...
     * @param array              $config
     */
    public function __construct(\Mailster\Aws\AwsClientInterface $client, $operation, array $args, array $config)
    {
        $this->client = $client;
    ...
    {
        return \Mailster\GuzzleHttp\Promise\coroutine(function () use($handleResult) {
            $nextToken = null;
            do {
                ...
                if ($retVal !== null) {
                    (yield \Mailster\GuzzleHttp\Promise\promise_for($retVal));
                }
            } while ($nextToken);
        });
    }
    ...
    public function search($expression)
    {
        ...
        return flatmap($this, function (\Mailster\Aws\Result $result) use($expression) {
    ...
    private function determineNextToken(\Mailster\Aws\Result $result)
    {
