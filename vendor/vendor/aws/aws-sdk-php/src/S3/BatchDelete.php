...
use Mailster\Aws\AwsClientInterface;
use Mailster\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 */
class BatchDelete implements \Mailster\GuzzleHttp\Promise\PromisorInterface
{
    private $bucket;
     ...
     * @return BatchDelete
     */
    public static function fromListObjects(\Mailster\Aws\AwsClientInterface $client, array $listObjectsParams, array $options = [])
    {
        $iter = $client->getPaginator('ListObjects', $listObjectsParams);
        $bucket = $listObjectsParams['Bucket'];
        $fn = function (\Mailster\Aws\S3\BatchDelete $that) use($iter) {
            return $iter->each(function ($result) use($that) {
                ...
                return $promises ? \Mailster\GuzzleHttp\Promise\all($promises) : null;
            });
        };
     ...
     * @return BatchDelete
     */
    public static function fromIterator(\Mailster\Aws\AwsClientInterface $client, $bucket, \Iterator $iter, array $options = [])
    {
        $fn = function (\Mailster\Aws\S3\BatchDelete $that) use($iter) {
            return \Mailster\GuzzleHttp\Promise\coroutine(function () use($that, $iter) {
                foreach ($iter as $obj) {
     ...
     */
    private function __construct(\Mailster\Aws\AwsClientInterface $client, $bucket, callable $promiseFn, array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
    ...
    private function flushQueue()
    {
        ...
        return $this->client->executeAsync($command)->then(function ($result) {
            if (!empty($result['Errors'])) {
                throw new \Mailster\Aws\S3\Exception\DeleteMultipleObjectsException($result['Deleted'] ?: [], $result['Errors']);
            }
            return $result;
        });
    }
    ...
    private function createPromise()
    {
        ...
        return $promise->then(function () use($cleanup) {
            return \Mailster\GuzzleHttp\Promise\promise_for($this->flushQueue())->then($cleanup);
