...
trait S3ClientTrait
{
    ...
    {
        return (new \Mailster\Aws\S3\ObjectUploader($this, $bucket, $key, $body, $acl, $options))->promise();
    }
    /**
    ...
    public function copyAsync($fromB, $fromK, $destB, $destK, $acl = 'private', array $opts = [])
    {
        ...
        return (new \Mailster\Aws\S3\ObjectCopier($this, $source, $destination, $acl, $opts))->promise();
    }
    /**
    ...
    {
        \Mailster\Aws\S3\StreamWrapper::register($this);
    }
    /**
    ...
    {
        if (!$prefix && !$regex) {
            return new \Mailster\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException('A prefix or regex is required.'));
        }
        $params = ['Bucket' => $bucket, 'Prefix' => $prefix];
        $iter = $this->getIterator('ListObjects', $params);
        if ($regex) {
            $iter = \Mailster\Aws\filter($iter, function ($c) use($regex) {
                return \preg_match($regex, $c['Key']);
            });
        }
        return \Mailster\Aws\S3\BatchDelete::fromIterator($this, $bucket, $iter, $options)->promise();
    }
    /**
    ...
    public function uploadDirectoryAsync($directory, $bucket, $keyPrefix = null, array $options = [])
    {
        ...
        return (new \Mailster\Aws\S3\Transfer($this, $directory, $d, $options))->promise();
    }
    /**
    ...
    public function downloadBucketAsync($directory, $bucket, $keyPrefix = '', array $options = [])
    {
        ...
        return (new \Mailster\Aws\S3\Transfer($this, $s, $directory, $options))->promise();
    }
    /**
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        return $handler($command)->then(static function (\Mailster\Aws\ResultInterface $result) {
            return $result['@metadata']['headers']['x-amz-bucket-region'];
        }, function (\Mailster\Aws\Exception\AwsException $e) {
    ...
    private function determineBucketRegionFromExceptionBody(\Mailster\Psr\Http\Message\ResponseInterface $response)
    {
        try {
     ...
     */
    private function checkExistenceWithCommand(\Mailster\Aws\CommandInterface $command)
    {
        try {
            $this->execute($command);
            return \true;
        } catch (\Mailster\Aws\S3\Exception\S3Exception $e) {
     ...
     */
    public abstract function execute(\Mailster\Aws\CommandInterface $command);
    /**
