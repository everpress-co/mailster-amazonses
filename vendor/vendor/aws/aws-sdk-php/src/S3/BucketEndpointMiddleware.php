...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\Mailster\Psr\Http\Message\RequestInterface $request, \Mailster\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \Mailster\Aws\S3\S3Client::encodeKey(\rawurldecode($path));
        }
