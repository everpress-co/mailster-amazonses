...
trait MultipartUploadingTrait
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\Mailster\Aws\S3\S3ClientInterface $client, $bucket, $key, $uploadId)
    {
        $state = new \Mailster\Aws\Multipart\UploadState(['Bucket' => $bucket, 'Key' => $key, 'UploadId' => $uploadId]);
    ...
    protected abstract function extractETag(\Mailster\Aws\ResultInterface $result);
    ...
    protected function determinePartSize()
    {
        ...
        $partSize = $this->getConfig()['part_size'] ?: \Mailster\Aws\S3\MultipartUploader::PART_MIN_SIZE;
        // Adjust the part size to be larger for known, x-large uploads.
        if ($sourceSize = $this->getSourceSize()) {
            $partSize = (int) \max($partSize, \ceil($sourceSize / \Mailster\Aws\S3\MultipartUploader::PART_MAX_NUM));
        }
        // Ensure that the part size follows the rules: 5 MB <= size <= 5 GB.
        if ($partSize < \Mailster\Aws\S3\MultipartUploader::PART_MIN_SIZE || $partSize > \Mailster\Aws\S3\MultipartUploader::PART_MAX_SIZE) {
