...
use Mailster\Aws\S3\MultipartUploader;
use Mailster\Aws\S3\S3ClientInterface;
use Mailster\GuzzleHttp\Promise;
/**
 * Encapsulates the execution of a multipart upload of an encrypted object to S3.
 */
class S3EncryptionMultipartUploader extends \Mailster\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \Mailster\Aws\Crypto\AbstractCryptoClient::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \Mailster\GuzzleHttp\Promise\promise_for($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
