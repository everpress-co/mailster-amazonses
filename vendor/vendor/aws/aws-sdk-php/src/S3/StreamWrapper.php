...
use Mailster\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
     ...
     */
    public static function register(\Mailster\Aws\S3\S3ClientInterface $client, $protocol = 's3', \Mailster\Aws\CacheInterface $cache = null)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \Mailster\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \Mailster\GuzzleHttp\Psr7\mimetype_from_filename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \Mailster\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\Mailster\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \Mailster\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return \true;
    }
    private function openWriteStream()
    {
        $this->body = new \Mailster\GuzzleHttp\Psr7\Stream(\fopen('php://temp', 'r+'));
        return \true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \Mailster\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
