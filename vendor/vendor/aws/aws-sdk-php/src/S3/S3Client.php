...
use Mailster\Aws\Api\DocModel;
use Mailster\Aws\Api\Service;
use Mailster\Aws\AwsClient;
use Mailster\Aws\CacheInterface;
use Mailster\Aws\ClientResolver;
use Mailster\Aws\Command;
use Mailster\Aws\Exception\AwsException;
use Mailster\Aws\HandlerList;
use Mailster\Aws\Middleware;
use Mailster\Aws\Retry\QuotaManager;
use Mailster\Aws\RetryMiddleware;
use Mailster\Aws\ResultInterface;
use Mailster\Aws\CommandInterface;
use Mailster\Aws\RetryMiddlewareV2;
use Mailster\Aws\S3\UseArnRegion\Configuration;
use Mailster\Aws\S3\UseArnRegion\ConfigurationInterface;
use Mailster\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use Mailster\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use Mailster\GuzzleHttp\Exception\RequestException;
use Mailster\GuzzleHttp\Promise\Promise;
use Mailster\GuzzleHttp\Promise\PromiseInterface;
 ...
 */
class S3Client extends \Mailster\Aws\AwsClient implements \Mailster\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    ...
    public function __construct(array $args)
    {
        if (!isset($args['s3_us_east_1_regional_endpoint'])) {
            $args['s3_us_east_1_regional_endpoint'] = \Mailster\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider();
        } elseif ($args['s3_us_east_1_regional_endpoint'] instanceof \Mailster\Aws\CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \Mailster\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\Mailster\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\Mailster\Aws\S3\ApplyChecksumMiddleware::wrap(), 's3.checksum');
        $stack->appendBuild(\Mailster\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\Mailster\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\Mailster\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }
        $stack->appendBuild(\Mailster\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null]), 's3.bucket_endpoint_arn');
        $stack->appendSign(\Mailster\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\Mailster\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\Mailster\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public static function _apply_use_arn_region($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        if ($value instanceof \Mailster\Aws\CacheInterface) {
            $value = \Mailster\Aws\S3\UseArnRegion\ConfigurationProvider::defaultProvider($args);
        }
        if (\is_callable($value)) {
            $value = $value();
        }
        if ($value instanceof \Mailster\GuzzleHttp\Promise\PromiseInterface) {
            $value = $value->wait();
        }
        if ($value instanceof \Mailster\Aws\S3\UseArnRegion\ConfigurationInterface) {
            $args['use_arn_region'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['use_arn_region'] = new \Mailster\Aws\S3\UseArnRegion\Configuration($value);
        }
    }
    public function createPresignedRequest(\Mailster\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\Mailster\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \Mailster\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\Mailster\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\Mailster\Aws\Command $command, $request = null) use($handler) {
    ...
    {
        return static function (callable $handler) {
            return function (\Mailster\Aws\CommandInterface $command, \Mailster\Psr\Http\Message\RequestInterface $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\Mailster\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\Mailster\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \Mailster\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \Mailster\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \Mailster\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                $decider = function ($retries, $command, $request, $result, $error) use($decider, $maxRetries) {
                    ...
                    if ($error instanceof \Mailster\Aws\Exception\AwsException && $retries < $maxRetries) {
                        ...
                        if ($error->getPrevious() instanceof \Mailster\GuzzleHttp\Exception\RequestException) {
                ...
                $delay = [\Mailster\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\Mailster\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \Mailster\Aws\RetryMiddlewareV2::createDefaultDecider(new \Mailster\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\Mailster\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \Mailster\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
                    $isRetryable = $defaultDecider($attempts, $cmd, $result);
                    if (!$isRetryable && $result instanceof \Mailster\Aws\Exception\AwsException && $attempts < $config->getMaxAttempts()) {
                        if (!empty($result->getResponse()) && \strpos($result->getResponse()->getBody(), 'Your socket connection to the server') !== \false) {
                            $isRetryable = \false;
                        }
                        if ($result->getPrevious() instanceof \Mailster\GuzzleHttp\Exception\RequestException && $cmd->getName() !== 'CompleteMultipartUpload') {
                            $isRetryable = \true;
                        }
                    }
                    return $isRetryable;
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \Mailster\Aws\HandlerList $list)
    {
        \Mailster\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \Mailster\Aws\S3\GetBucketLocationParser(new \Mailster\Aws\S3\AmbiguousSuccessParser(new \Mailster\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \Mailster\Aws\Api\Service($api, \Mailster\Aws\Api\ApiProvider::defaultProvider()), new \Mailster\Aws\Api\DocModel($docs)];
    }
    /**
     * @internal
     * @codeCoverageIgnore
