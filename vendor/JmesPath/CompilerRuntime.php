...
class CompilerRuntime
{
     ...
     */
    public function __construct($dir = null, \Mailster\Aws3\JmesPath\Parser $parser = null)
    {
        $this->parser = $parser ?: new \Mailster\Aws3\JmesPath\Parser();
        $this->compiler = new \Mailster\Aws3\JmesPath\TreeCompiler();
        $dir = $dir ?: sys_get_temp_dir();
        if (!is_dir($dir) && !mkdir($dir, 0755, true)) {
            throw new \RuntimeException("Unable to create cache directory: {$dir}");
        }
        $this->cacheDir = realpath($dir);
        $this->interpreter = new \Mailster\Aws3\JmesPath\TreeInterpreter();
    }
    /**
